<? 
    ...
    //--> Estoy navegando la ZONA con un editable... 
        ...
        if($abms > -1){ 
            $listado_notas = $this->cargar_objeto("objeto_cuadro",0); 
            if($listado_notas > -1){ 
                $this->objetos[$abms]->procesar(); 
                $cargar_ef = array( "clase_cons"=>$editable[1]); 
                $this->objetos[$abms]->cargar_estado_ef($cargar_ef); 
                $this->objetos[$listado_notas]->cargar_datos( 
                        array("(clase_consumidora_proyecto = '".$editable[0]."') ", 
                                "(clase_consumidora = '".$editable[1]."') ") ); 
				enter();
