...
<tr> 
	...
	<a href="<? echo $this->vinculador->generar_solicitud("toba","/admin/apex/elementos/dimension_tipo") ?>" target="<? echo  apex_frame_centro ?>" class="list-obj">
	&nbsp;<? echo recurso::imagen_apl("dimension_tipo.gif",true) ?>&nbsp;
	</a>
	</td>
    <td width="98%" class="lista-obj-titulo" >DIMENSIONES</td>
	 <td width="2%"  class='lista-obj-titulo'>
	<a href="<? echo $this->vinculador->generar_solicitud("toba","/admin/dimensiones/propiedades") ?>" target="<? echo  apex_frame_centro ?>" class="list-obj">
...
<?	if($rs->fields["perfil"]!="no"){ ?>
          <td width="2%" class='lista-obj-dato5'>
		 	<a href="<? echo $this->vinculador->generar_solicitud("toba","/admin/dimensiones/propiedades",array( apex_hilo_qs_zona => $rs->fields['dim_proy'] .apex_qs_separador. $rs->fields['dim_id'] )) ?>" target="<? echo  apex_frame_centro ?>">
				<img src="<? echo recurso::imagen_apl("dimension.gif") ?>" alt="<? echo trim($rs->fields["dim_id"])?>" border="0">
			</a>
  	  </td>
          <td width="2%" class='lista-obj-dato5'>
		 	<a href="<? echo $this->vinculador->generar_solicitud("toba","/admin/dimensiones/probar",array( apex_hilo_qs_zona => $rs->fields['dim_proy'] .apex_qs_separador. $rs->fields['dim_id'] )) ?>" target="<? echo  apex_frame_centro ?>">
