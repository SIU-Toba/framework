...
$maximo = 10;//Maximo nivel de anidacion...
	...
	$parametros["sql"] = "SELECT item,item from apex_item WHERE carpeta = 1 AND proyecto = '".$this->hilo->obtener_proyecto()."' AND item <> '';";	
	...
	if(acceso_post()){
		...
		if($temp!='NULL') {
			...
			$this->hilo->persistir_dato_global("carpeta",$temp);
			$where_carpeta = " AND item LIKE '". $temp ."%'";
		}else{
		...
		if ($temp = $this->hilo->recuperar_dato_global("carpeta")) {
			$tipo_clase->cargar_estado($temp);
			$where_carpeta = " AND item LIKE '". $temp ."%'";
		}
		//No viene de POST y no hay un dato en el hilo
		else{
			$where_carpeta = "";
		}	
	}	
...
?>
          <td  class='cat-arbol-carpeta' width='2%'>
			<a href="<? echo $this->vinculador->generar_solicitud("toba","/admin/items/carpeta_propiedades",array( apex_hilo_qs_zona => $rs->fields['item_proyecto'] .apex_qs_separador. $rs->fields["item"])) ?>" target="<? echo  apex_frame_centro ?>"  class='cat-item'>
		  ...
		  <? echo strrchr($rs->fields["item"],"/") ?></td>
...
?>
          <td  class='cat-arbol-carpeta-info' width='2%'>
			<a href="<? echo $this->vinculador->generar_solicitud("toba","/admin/items/carpeta_ordenar", array("padre_p"=>$rs->fields["item_proyecto"], "padre_i"=>$rs->fields["item"]) ) ?>" target="<? echo  apex_frame_centro ?>"  class='cat-item'>
          ...
          <td  class='cat-arbol-carpeta-info' width='2%'>
			<a href="<? echo $this->vinculador->generar_solicitud("toba","/admin/items/carpeta_propiedades", array("padre_p"=>$rs->fields["item_proyecto"], "padre_i"=>$rs->fields["item"]) ) ?>" target="<? echo  apex_frame_centro ?>"  class='cat-item'>
			<? echo recurso::imagen_apl("items/carpeta_nuevo.gif",true,null,null,"Crear SUBCARPETA en esta rama del CATALOGO") ?></a>
		  </td>
          <td  class='cat-arbol-carpeta-info' width='2%'>
			<a href="<? echo $this->vinculador->generar_solicitud("toba","/admin/items/propiedades", array("padre_p"=>$rs->fields["item_proyecto"], "padre_i"=>$rs->fields["item"]) ) ?>" target="<? echo  apex_frame_centro ?>"  class='cat-item'>
...
?>
          <td  class='cat-arbol-item'  width='2%'>
			<a href="<? echo $this->vinculador->generar_solicitud("toba","/admin/items/propiedades",array( apex_hilo_qs_zona => $rs->fields['item_proyecto'] .apex_qs_separador. $rs->fields["item"]))?>" target="<? echo  apex_frame_centro ?>"  class='cat-item'>
