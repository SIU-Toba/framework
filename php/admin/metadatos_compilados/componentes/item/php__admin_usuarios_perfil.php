<?
//Generador: compilador_proyecto.php

class php__admin_usuarios_perfil
{
	function get_metadatos()
	{
		return array (
  'info' => 
  array (
    'item_proyecto' => 'toba',
    'item' => '/admin/usuarios/perfil',
    'item_nombre' => 'PERFIL de DATOS - Propiedades',
    'item_descripcion' => NULL,
    'item_act_buffer_proyecto' => 'toba',
    'item_act_buffer' => '0',
    'item_act_patron_proyecto' => 'toba',
    'item_act_patron' => 'especifico',
    'item_act_patron_script' => 'NO APLICABLE',
    'item_act_accion_script' => 'acciones/admin/usuarios/perfil.php',
    'item_solic_tipo' => 'browser',
    'item_solic_registrar' => NULL,
    'item_solic_obs_tipo_proyecto' => NULL,
    'item_solic_obs_tipo' => NULL,
    'item_solic_observacion' => NULL,
    'item_solic_cronometrar' => NULL,
    'item_parametro_a' => NULL,
    'item_parametro_b' => NULL,
    'item_parametro_c' => NULL,
    'tipo_pagina_clase' => 'tp_normal',
    'tipo_pagina_archivo' => 'nucleo/browser/tipo_pagina/tp_normal.php',
    'item_include_arriba' => NULL,
    'item_include_abajo' => NULL,
    'item_zona_proyecto' => 'toba',
    'item_zona' => 'zona_usuario_perfil',
    'item_zona_archivo' => 'nucleo/browser/zona/zona_perfil_datos.php',
    'item_publico' => NULL,
    'item_existe_ayuda' => NULL,
    'carpeta' => '0',
    'menu' => NULL,
    'orden' => '610',
    'publico' => NULL,
    'crono' => NULL,
    'solicitud_tipo' => 'browser',
  ),
  'info_objetos' => 
  array (
    0 => 
    array (
      'objeto_proyecto' => 'toba',
      'objeto' => '128',
      'objeto_nombre' => 'USUARIO - Perfil',
      'objeto_subclase' => NULL,
      'objeto_subclase_archivo' => NULL,
      'orden' => '0',
      'clase_proyecto' => 'toba',
      'clase' => 'objeto_mt_abms',
      'clase_archivo' => 'nucleo/browser/clases/objeto_mt_s_abm.php',
      'fuente_proyecto' => 'toba',
      'fuente' => 'instancia',
      'fuente_motor' => 'postgres7',
      'fuente_host' => NULL,
      'fuente_usuario' => NULL,
      'fuente_clave' => NULL,
      'fuente_base' => 'No aplicable',
    ),
    1 => 
    array (
      'objeto_proyecto' => 'toba',
      'objeto' => '210',
      'objeto_nombre' => 'USUARIO - Perfil (dimensiones)',
      'objeto_subclase' => NULL,
      'objeto_subclase_archivo' => NULL,
      'orden' => '1',
      'clase_proyecto' => 'toba',
      'clase' => 'objeto_mt_abms',
      'clase_archivo' => 'nucleo/browser/clases/objeto_mt_s_abm.php',
      'fuente_proyecto' => 'toba',
      'fuente' => 'instancia',
      'fuente_motor' => 'postgres7',
      'fuente_host' => NULL,
      'fuente_usuario' => NULL,
      'fuente_clave' => NULL,
      'fuente_base' => 'No aplicable',
    ),
    2 => 
    array (
      'objeto_proyecto' => 'toba',
      'objeto' => '211',
      'objeto_nombre' => 'USUARIO - Perfil (dimensiones)',
      'objeto_subclase' => 'objeto_cuadro_perfil',
      'objeto_subclase_archivo' => 'acciones/admin/usuarios/perfil_clases.php',
      'orden' => '2',
      'clase_proyecto' => 'toba',
      'clase' => 'objeto_cuadro',
      'clase_archivo' => 'nucleo/browser/clases/objeto_cuadro.php',
      'fuente_proyecto' => 'toba',
      'fuente' => 'instancia',
      'fuente_motor' => 'postgres7',
      'fuente_host' => NULL,
      'fuente_usuario' => NULL,
      'fuente_clave' => NULL,
      'fuente_base' => 'No aplicable',
    ),
  ),
);
	}

}
?>