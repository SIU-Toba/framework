...
class comando_toba extends comando
{
			...
			if ($seleccion_multiple) {
				foreach ($sel as $path) {
					if (isset($seleccion[$proyectos[$path]])) {
						throw new toba_error('ERROR: Una instancia no soporta contener el mismo proyecto m�s de una vez');
					}
	...
	*/
        ...
        protected function definir_clave_usuario_admin($param)
	{
		...
		if ($es_invalido) {
			$this->consola->mensaje('Se procede con un password no v�lido bajo su responsabilidad' , true);
	...
	protected function definir_usuario_admin($param)
	{
		...
		if ($result['invalido']) {
			toba::logger()->error("Se selecciono toba como nombre del usuario administrador, ya que uno v�lido no fue provisto");
			return 'toba';
		}
