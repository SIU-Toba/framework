...
class toba_manejador_archivos
{
	...
	/**
	 * Similar al file_exists de php pero incluye al include_path en la b�squeda
	 * @param string $file
	 * @return boolean
	 */
	...
	/**
	 * Retorna un path convertido a la plataforma actual de ejecuci�n (unix o windows)
	...
	static function get_archivos_directorio( $directorio, $patron = null, $recursivo_subdir = false, &$exclude_dirs = array() )
	{
		...
		if( ! is_dir( $directorio ) ) {
			...
			throw new toba_error('BUSCAR ARCHIVOS: El directorio solicitado no es v�lido o no tiene permisos');
		}
	...
	static function get_subdirectorios( $directorio )
	{
		...
		if( ! is_dir( $directorio ) ) {
			...
			throw new toba_error('BUSCAR SUBDIRECTORIOS: El directorio solicitado no es v�lido o no tiene permisos');
		}
		$dir = opendir($directorio);
		if ($dir !== false) {
		   while (false	!==	( $archivo = readdir( $dir ) ) )	{
	...
	static function copiar_directorio( $origen, $destino, $excepciones=array(), $manejador_interface = null, $copiar_ocultos=true )
	{
		if( ! is_dir( $origen ) ) {
			...
			throw new toba_error('COPIAR DIRECTORIO: El directorio de origen no es v�lido o no tiene permisos');
		}
		$ok = true;
		if( ! is_dir( $destino ) ) {
			$ok = mkdir($destino) && $ok;
	...
	static function eliminar_directorio( $directorio )
	{
		if( ! is_dir( $directorio ) ) {
			...
			throw new toba_error('ELIMINAR DIRECTORIO: El directorio solicitado no es v�lido o no tiene permisos');
		}
		$ok = true;
		$dir = opendir( $directorio );
		while ( false !== ($archivo = readdir($dir))) {
