...
{
    private static $caracteres_invalidos = array('*', '?', '/', '>', '<', '"', "'", ':', '|');
     ...
     * @throws toba_error
     */
    public static function buscar_archivos_directorio_recursivo($directorio, &$exclude_dirs = array())
    {
        if (! is_dir($directorio)) {
            toba_logger::instancia()->error("BUSCAR ARCHIVOS: El directorio '$directorio' es INVALIDO");
            throw new toba_error('BUSCAR ARCHIVOS: El directorio solicitado no es válido o no tiene permisos');
     ...
     * @return boolean
     */
    public static function chmod_recursivo($path, $filemode)
    {
        if (!is_dir($path)) {
     ...
     * @return boolean
     */
    public static function comprimir_archivo($src, $level = 5, $dst = false)
    {
        if ($dst == false) {
            $dst = $src.".gz";
        }
     ...
     * @return boolean
     */
    public static function es_directorio_vacio($dir)
    {
        $dh = opendir($dir);
        if ($dh !== false) {
            while ($file = readdir($dh)) {
     ...
     * @return boolean
     */
    public static function es_writable($path)
    {
        if (substr($path, -1) == '/') {
