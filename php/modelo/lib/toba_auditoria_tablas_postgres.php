...
class toba_auditoria_tablas_postgres
{
    ...
    public function __construct(toba_db $conexion, $schema_origen = 'public', $schema_logs = 'public_auditoria', $schema_toba = null)
    {
    ...
    public function agregar_tabla($nombre)
    {
        $this->tablas[] = $nombre;
    }

    public function set_esquema_origen($esquema)
    {
        $this->schema_origen = $esquema;
    }
    public function set_esquema_toba($esquema)
    {
        $this->schema_toba = $esquema;
    }

    public function set_esquema_logs($esquema)
    {
        $this->schema_logs = $esquema;
    }

    public function set_triggers_eliminacion_forzada($fuerza = false)
    {
        $this->fuerza_eliminacion_triggers = $fuerza;
    }

    public function reset_tablas()
    {
        unset($this->tablas);
        $this->tablas = array();
    }
    ...
    public function purgar($lapso_tiempo = 0)
    {
        foreach ($this->tablas as $t) {
    ...
    public function desactivar_triggers_auditoria()
    {
    ...
    public function activar_triggers_auditoria()
    {
    ...
    public function get_triggers_activos($schema = null)
    {
        if (is_null($schema)) {
            $schema = $this->schema_origen;
        }
    ...
    public function get_tablas_triggers_desactivados($schema = null)
    {
        if (is_null($schema)) {
            $schema = $this->schema_origen;
        }
    ...
    public function set_estado_activacion_triggers($tablas, $schema, $estado)
    {
        $sql = array();
    ...
    public function get_datos($tabla, $filtro = array())
    {
        $where = 'true';
        $order = '';
    ...
    public function get_campos_claves($tabla)
    {
    ...
    public function purgar_datos($tabla, $lapso_tiempo)
    {
    ...
    public function migrar_estructura_campos_toba_2_4()
    {
        foreach ($this->tablas as $t) {
            $nombre = $this->prefijo.$t;
            if ($this->conexion->existe_tabla($this->schema_logs, $nombre)) {
    ...
    public function migrar_estructura_campos_toba_3_1()
    {
        foreach ($this->tablas as $t) {
            $nombre = $this->prefijo.$t;
            if ($this->conexion->existe_tabla($this->schema_logs, $nombre)) {
