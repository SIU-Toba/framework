...
class migracion_0_9_0 extends migracion_toba
{
	...
	*/
	...
	function instalacion__construir_inis()
	{
		...
		if( ! instalacion::existe_info_bases() ) {
			...
			$this->manejador_interface->mensaje("la definicion de BASES se encuentra ahora en '" . 
																	instalacion::archivo_info_bases() . "'");	
		} else {
		...
		if( ! instalacion::existe_info_basica() ) {
			...
			$this->manejador_interface->mensaje( "ya existe una archivo '" . instalacion::archivo_info_basica() . "'" );
		}
	...
	{
		$sql = array();
		$sql[] = "ALTER TABLE apex_proyecto ADD COLUMN version_toba varchar(15);";
		//--- Se asume que los proyectos estan en la primer version de toba con este esquema
		$sql[] = "UPDATE apex_proyecto SET version_toba='0.8.3' WHERE version_toba IS NULL";
