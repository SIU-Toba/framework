...
class toba_ei_formulario_ml_molde extends toba_ei_formulario_molde
{
	...
	{
		$this->datos->tabla('prop_basicas')->set_fila_columna_valor(0,'filas_agregar',$clave);
	}
	...
	function agregar_filas_js($tipo=1)
	{
		...
		if(in_array($tipo, $tipos_validos)) {
			throw new toba_error('MOLDE ML: El tipo de agregado de filas especificado no es valido');
		}
		$this->agregar_filas();
	...
	function set_analisis_cambios($tipo)
	{
		...
		if(in_array($tipo, $tipos_validos)) {
			throw new toba_error('MOLDE ML: El tipo de analisis especificado no es valido');	
		}
