...
class toba_pers_xml_generador_componentes extends toba_pers_xml_generador
{
     ...
     */
    public function generar_componentes_nuevas($path, &$data)
    {
        if (empty($data)) {
            return;
        }
        ...
        foreach (array_keys($data) as $tipo) {
            foreach ($data[$tipo] as $comp) {
                $path_componente = $this->get_path_componente($path_nuevos, $comp['tipo'], $comp['id']['componente']);

                $this->agregar_al_plan($comp['id']['componente'], $path_componente);

                $path_absoluto = $this->get_path_componente($path . $path_nuevos, $comp['tipo'], $comp['id']['componente']);
     ...
     * @param array $data componentes a exportar
     */
    public function generar_componentes_modificadas($path, &$data)
    {
        if (empty($data)) {
            return;
        }
        ...
        foreach (array_keys($data) as $tipo) {
            foreach ($data[$tipo] as $comp) {
                $path_componente = $this->get_path_componente($path_modificados, $comp['tipo'], $comp['id']['componente']);

                $this->agregar_al_plan($comp['id']['componente'], $path_componente);

                $path_absoluto = $this->get_path_componente($path . $path_modificados, $comp['tipo'], $comp['id']['componente']);
    ...
    public function generar_componentes_borradas($path, &$data)
    {
        if (empty($data)) {
            return;
        }
        ...
        foreach (array_keys($data) as $tipo) {
            foreach ($data[$tipo] as $comp) {
                $path_componente = $this->get_path_componente($path_borrados, $comp['tipo'], $comp['id']['componente']);
                $this->agregar_al_plan($comp['id']['componente'], $path_componente);

                $path_absoluto = $this->get_path_componente($path . $path_borrados, $comp['tipo'], $comp['id']['componente']);
