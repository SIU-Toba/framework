...
{
    protected static $conexion_externa;
    public const db_encoding_estandar = 'LATIN1';
    public const info_basica = 'instalacion.ini';
    public const info_basica_titulo = 'Configuracion de la INSTALACIÓN';
    public const info_bases = 'bases.ini';
    public const info_bases_titulo = 'Configuracion de BASES de DATOS';
    ...
    public function __construct()
    {
        ...
        toba_logger::instancia()->debug('INSTALACIÓN "'.$this->dir.'"');
    }

    public function cargar_info_ini($forzar_recarga = false)
    {
        if ($forzar_recarga || !$this->ini_cargado) {
            ...
            if (! is_file($archivo_ini_bases)) {
                toba_logger::instancia()->error("INSTALACIÓN: La instalación '".toba_dir()."' es inválida. (El archivo de configuración '$archivo_ini_bases' no existe)");
                throw new toba_error('INSTALACIÓN: La instalación es inválida. (El archivo de configuración no existe) Revise el log');
            } else {
                //  BASE
                $pendientes = array();
                $tc = toba::config();
            ...
            if (! is_file($archivo_ini_instalacion)) {
                toba_logger::instancia()->error("INSTALACIÓN: La instalación '".toba_dir()."' es inválida. (El archivo de configuración '$archivo_ini_instalacion' no existe)");
                throw new toba_error('INSTALACIÓN: La instalación es inválida. (El archivo de configuración no existe) Revise el log');
            }
    ...
    public function get_instancia($id)
    {
        return toba_modelo_catalogo::instanciacion()->get_instancia(
            $id,
            $this->manejador_interface
        );
    }

    public function get_lista_instancias()
    {
        return toba_modelo_instancia::get_lista();
    }

    public function existe_instancia($id)
    {
        return toba_modelo_instancia::existe_carpeta_instancia($id);
    }

    public static function set_conexion_externa($base)
    {
     ...
     */
    public function get_path_carpeta_instalacion()
    {
    ...
    */
    public function get_id_grupo_desarrollo()
    {
    ...
    */
    public function vincula_arai_usuarios()
    {
     ...
     */
    public function chequea_sincro_svn()
    {
        $chequea = false;
    ...
    */
    public function get_claves_encriptacion()
    {
    ...
    public function get_archivos_certificado_ssl()
    {
    ...
    public function get_parametros_base($id_base)
    {
        $this->cargar_info_ini();
        if (isset($this->ini_bases[$id_base])) {
            return $this->ini_bases[$id_base];
        } else {
            toba_logger::instancia()->error("INSTALACIÓN: La base '$id_base' no existe en el archivo bases.ini");
            throw new toba_error('INSTALACIÓN: La base solicitada no existe en el archivo bases.ini');
        }
    }

    public function existe_base_datos_definida($id_base)
    {
        $this->cargar_info_ini();
        return isset($this->ini_bases[$id_base]);
    }

    public function hay_bases()
    {
        $this->cargar_info_ini();
        return count($this->ini_bases) > 0 ;
    }

    public function get_lista_bases()
    {
        $this->cargar_info_ini();
        return array_keys($this->ini_bases);
    }

    public function agregar_db($id_base, $parametros)
    {
     ...
     */
    public function actualizar_db($id_base, $parametros)
    {
        self::validar_parametros_db($parametros);
        $ini = new toba_ini(self::archivo_info_bases());
    ...
    */
    public function conectar_base($nombre)
    {
    ...
    */
    public function crear_base_datos($nombre, $con_encoding = false, $cant_intentos = 3)
    {
    ...
    *	@param string $nombre Nombre de la base
    */
    public function borrar_base_datos($nombre)
    {
        $info_db = $this->get_parametros_base($nombre);
    ...
    *	@param string $nombre Nombre de la base
    */
    public function existe_base_datos($nombre, $otra_info = array(), $mostrar_salida = false, $schema = null)
    {
        try {
            $this->ini_cargado = false;
            $info_db = $this->get_parametros_base($nombre);
    ...
    */
    public function conectar_base_parametros($parametros)
    {
    ...
    *	@param string $nombre Nombre de la base
    */
    public function determinar_encoding($id_base)
    {
        /*
    ...
    public static function crear($id_grupo_desarrollo, $alias_nucleo, $nombre, $es_produccion = 0)
    {
    ...
    public static function crear_archivo_apache($alias_nucleo)
    {
    ...
    public static function get_archivo_alias_apache()
    {
     ...
     */
    public static function agregar_alias_apache($alias, $dir, $instancia, $id_proyecto, $pers = false)
    {
        $archivo = self::get_archivo_alias_apache();
    ...
    public static function existe_alias_apache($id_proyecto, $pers = false)
    {
        $archivo = self::get_archivo_alias_apache();
    ...
    public static function quitar_alias_apache($id_proyecto, $pers = false)
    {
        $archivo = self::get_archivo_alias_apache();
        $conf = file_get_contents($archivo);
    ...
    public static function dir_base()
    {
    ...
    */
    public static function crear_directorio()
    {
    ...
    public static function borrar_directorio()
    {
        if (is_dir(self::dir_base())) {
    ...
    */
    public static function crear_directorio_proyectos()
    {
     ...
     */
    public static function get_lista_proyectos()
    {
        $dir = toba_dir() .'/proyectos';
        return toba_manejador_archivos::get_subdirectorios($dir);
    }

    //-- Archivo de CONFIGURACION de la INSTALACIÓN  --------------------------------------

    /**
    * Crea el archivo con la informacion basica sobre la instalacion
    */
    public static function crear_info_basica($clave_qs, $clave_db, $id_grupo_desarrollo, $editor, $url, $nombre_inst, $es_produccion = 0)
    {
        $ini = new toba_ini();
     ...
     */
    public function cambiar_info_basica($datos)
    {
    ...
    */
    public static function existe_info_basica()
    {
    ...
    */
    public static function archivo_info_basica()
    {
    ...
    */
    public static function crear_info_bases($lista_bases = array())
    {
        $ini = new toba_ini();
        $ini->agregar_titulo(self::info_bases_titulo);
        foreach ($lista_bases as $id => $base) {
            ...
            if (isset($base['motor']) &&
                ...
                toba_logger::instancia()->error("La definición de la BASE '$id' es INCORRECTA.");
                throw new toba_error("La definición especificada para una BASE es INCORRECTA. Revise el log");
            }
        }
    ...
    private static function validar_parametros_db($parametros)
    {
        if (! is_array($parametros)) {
            throw new toba_error('INSTALACIÓN: Los parámetros definidos son incorrectos');
        } else {
            ...
            if (! in_array($parametros['motor'], $motores)) {
                toba_logger::instancia()->error("INSTALACIÓN: El motor tiene que pertenecer a la siguente lista: " . implode(', ', $motores));
                throw new toba_error('INSTALACIÓN: El motor especificado no se encuentra entre los reconocidos, revise el log');
            }
        }
    }

    public static function eliminar_db($id_base)
    {
        $ini = new toba_ini(self::archivo_info_bases());
        $ini->agregar_titulo(self::info_bases_titulo);
    ...
    public function existe_info_bases()
    {
        return (is_file(self::archivo_info_bases()));
    }

    public static function archivo_info_bases()
    {
     ...
     */
    public function set_id_desarrollador($id)
    {
    ...
    public function empaquetar_en_carpeta($destino, $librerias_en_uso = array(), $proyectos_en_uso = array())
    {
        $path_base = toba_dir();
    ...
    public function get_lista_excepciones_instalacion($path_base, $librerias_en_uso, $proyectos_en_uso)
    {
        $excepciones = array();
     ...
     */
    public function importar_migrar_proyecto($id_instancia, $id_proyecto, $dir_toba_viejo, $destino = null)
    {
        ...
        try {
            ...
            if (! file_exists($archivo_ini_bases)) {
                ...
                throw new toba_error('No se encuentra el archivo de configuración, revise el log');
            }
    ...
    public function migrar_version($version, $recursivo)
    {
     ...
     * @return toba_version
     */
    public static function get_version_actual()
    {
        return new toba_version(file_get_contents(toba_dir()."/VERSION"));
    }

    public function get_version_anterior()
    {
    ...
    public function get_compatibilidad_arai_cli()
    {
        $min = " 0.0.0";
        $max = "32765.9.9";
    ...
    public function vincula_arai_proyecto()
    {
