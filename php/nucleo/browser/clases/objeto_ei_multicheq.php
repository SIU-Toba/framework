...
class	objeto_ei_multicheq extends objeto_ei
/*	
	@@acceso: nucleo
	@@desc: Descripcion
*/	
{
	...
	function	objeto_ei_multicheq($id)
/*	
	@@acceso: nucleo
...
*/
	{
		...
		$sql["info_ei_multicheq"]["sql"] =" SELECT		sql as	sql,
										  claves	as					  claves,
...
*/	
	{
		...
		$sql = $this->info_ei_multicheq["sql"];
		...
		if(!$rs){
			...
			$this->registrar_info_proceso("la consulta no devolvio registros");
								...
								foreach($datos	as	$dato){
										  $id	= $dato[$this->info_ei_multicheq['claves']];	
										  $ef	=&	new ef_checkbox(	$this->id[1],			
																		  $this->nombre_formulario,
																		  $id,
																		  $dato[$this->info_ei_multicheq['descripcion']],
																		  null,
																		  null,
																		  null,
																		  $parametros_ef);
										 ...
										 if( $this->evaluar_chequeo_inicial($dato[$this->info_ei_multicheq['chequeado']]) )	
...
*/	
	{
                //ATENCION: Javascript para seleccionar todas las opcciones
					 if($this->info_ei_multicheq['forzar_chequeo']){
								echo " if(\n";	
							if(is_array($this->estado)){
								foreach(	array_keys($this->estado) as $linea	){	
