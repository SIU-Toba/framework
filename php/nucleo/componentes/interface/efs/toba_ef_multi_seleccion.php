	...
	protected function validar_estado_particular($estado)
	{
		if (is_array($this->dato)) {
			...
			if (count($estado) <> $cant_datos) {
				throw new toba_excepcion("Ha intentado cargar el combo '{$this->id}' con un array que posee un formato inadecuado " .
		...
		if (is_array($this->dato)) {
			...
			foreach ($estado as $seleccion) {
				...
				if (count($valores) <> $cant_datos) {
					throw new toba_excepcion("Ha intentado cargar el ef '{$this->id}' con un array que posee un formato inadecuado " .
