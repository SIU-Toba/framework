...
abstract class toba_ei extends toba_componente
{
    ...
    public function __construct($definicion)
    {
     ...
     */
    public static function set_refresh_ejecuta_eventos($activado = true)
    {
     ...
     * @ignore
     */
    public function pre_configurar()
    {
    }
    ...
    */
    public function evento($id)
    {
        if (isset($this->_eventos_usuario_utilizados[$id])) {
            ...
            if (isset($this->_eventos_usuario[$id])) {
                toba_logger::instancia()->error($this->get_txt(). " El EVENTO '$id' no esta asociado actualmente al componente.");
                throw new toba_error_def(' El EVENTO no esta definido o asociado al componente. Revise el log');
            } else {
                toba_logger::instancia()->error($this->get_txt(). " El EVENTO '$id' no está definido.");
                throw new toba_error_def(' El EVENTO no esta definido o asociado al componente. Revise el log');
            }
        }
    }
     ...
     * @param string $id
     */
    public function agregar_evento($id)
    {
        if (isset($this->_eventos_usuario[ $id ])) {
            $this->_eventos_usuario_utilizados[ $id ] = $this->_eventos_usuario[ $id ];
        } else {
            toba_logger::instancia()->error($this->get_txt()." Se quiere agregar el EVENTO '$id', pero no está definido.");
            throw new toba_error_def(' Se quiere agregar un EVENTO no definido. Revise el log');
        }
    }
     ...
     * @param string $id
     */
    public function existe_evento($id)
    {
        $existe = false;
    ...
    public function eliminar_evento($id)
    {
        if (isset($this->_eventos_usuario[ $id ])) {
            ...
            toba_logger::instancia()->error($this->get_txt(). " Se quiere eliminar el EVENTO '$id', pero no está definido.");
            throw new toba_error_def(' Se quiere eliminar un EVENTO no definido. Revise el log');
        }
    }
     ...
     */
    public function set_disparo_eventos_condicionado_datos($disparo = true)
    {
     ...
     * @return integer
     */
    public function cant_eventos_sobre_fila()
    {
     ...
     * @ignore
     */
    public function borrar_memoria_eventos_atendidos()
    {
     ...
     */
    public function get_lista_eventos_usuario()
    {
     ...
     * @return array(toba_evento_usuario)
     */
    public function get_lista_eventos_internos()
    {
     ...
     * @param integer $fila
     */
    public function get_invocacion_evento_fila($evento, $fila, $clave_fila, $salida_como_vinculo = false, $param_extra = array())
    {
        $invoc_evt = '';
        $id = $evento->get_id();
        if (! $evento->esta_anulado()) { //Si el evento viene desactivado de la conf, no lo utilizo
            ...
            if (! $evento->esta_anulado()) {
                ...
                } elseif ($evento->posee_accionar_diferido()) {		//Si es un evento que no dispara submit inmediatamente (solo para el cuadro por ahora)
     ...
     */
    public function set_grupo_eventos_activo($grupo)
    {
     ...
     * @return string
     */
    public function get_grupo_eventos_activo()
    {
     ...
     * @return boolean
     */
    public function botonera_abajo()
    {
     ...
     * @return boolean
     */
    public function botonera_arriba()
    {
     ...
     */
    public function generar_botones($clase = '', $extra = '')
    {
        //----------- Generacion
        if ($this->hay_botones()) {
    ...
    */
    public function generar_boton($id_evento, $excluir_botonera = true, $retornar = false)
    {
     ...
     * @return boolean
     */
    public function tiene_puntos_control($evento)
    {
     ...
     * @return array
     */
    public function get_puntos_control($evento)
    {
        $ret = array();
     ...
     */
    public function agregar_notificacion($mensaje, $tipo = 'info')
    {
     ...
     * @param string $tipo Puede ser 'info', 'warning', 'error'
     */
    public function set_descripcion($desc, $tipo = 'info')
    {
     ...
     */
    public function set_modo_descripcion($tooltip = true)
    {
     ...
     */
    public function generar_html_barra_sup($titulo = null, $control_titulo_vacio = false, $estilo = "")
    {
     ...
     *
     */
    public function mostrar_barra_superior($estado = true)
    {
     ...
     * @ignore
     */
    public function get_html_barra_editor()
    {
        $salida = '';
        ...
        if (toba_editor::modo_prueba() && ($servicio == 'generar_html' || $servicio == 'html_parcial')) {
            ...
            $salida .= toba_editor::generar_zona_vinculos_componente(
                $this->_id,
                $this->_info['clase_editor_item'],
                $this->_info['clase'],
                $this->_info['subclase'] != ''
            );
     ...
     * @return string
     */
    public function get_id_objeto_js()
    {
        return $this->objeto_js;
    }
     ...
     */
    public function vista_impresion(toba_impresion $salida)
    {
     ...
     */
    public function xml_set_orientacion($orientacion = 'portrait')
    {
     ...
     */
    public function xml_set_logo($logo)
    {
        $this->xml_logo = $logo;
    }
    ...
    public function xml_set_titulo($titulo)
    {
     ...
     * @return unknown_type
     */
    public function xml_set_subtitulo($subtitulo)
    {
     ...
     * @param string $alto
     */
    public function xml_set_dim_pagina($ancho = false, $alto = false)
    {
     ...
     */
    public function xml_set_incluir_pie($incluir = true)
    {
     ...
     * @param string $pie
     */
    public function xml_set_pie($pie = false)
    {
     ...
     * @param string $alto
     */
    public function xml_set_alto_pie($alto = false)
    {
     ...
     * @param boolean $incluir default true
     */
    public function xml_set_incluir_cabecera($incluir = true)
    {
     ...
     */
    public function xml_set_cabecera($cabecera = false)
    {
     ...
     * @param string $alto
     */
    public function xml_set_alto_cabecera($alto = false)
    {
     ...
     */
    public function xml_set_margenes($margenes = array())
    {
     ...
     * @param array $atts
     */
    public function xml_set_atts_ei($atts = array())
    {
     ...
     * @param int $copias
     */
    public function xml_set_nro_copias($copias = 1)
    {
     ...
     */
    public function xml_get_att_comunes()
    {
        $xml = '';
     ...
     */
    public function xml_get_elem_comunes()
    {
        $xml = '';
     ...
     * @return string
     */
    public function xml_texto($texto, $atts = array())
    {
     ...
     * @return string
     */
    public function xml_tabla($datos = array(), $es_formulario = true)
    {
     ...
     * @return string
     */
    public function xml_imagen($src, $tipo = 'jpg', $titulo = false, $caption = false)
    {
     ...
     */
    public function xml_set_tabla_cols($cols = array())
    {
        if ($cols) {
            foreach ($cols as $col) {
     ...
     */
    public function xml_set_ns($xmlns, $url = '', $usar = true)
    {
        if ($xmlns == '' || $xmlns == null) {
            $this->xml_ns = '';
    ...
    public function xml_get_informacion_basica_vista()
    {
