...
class toba_ei_archivos extends toba_ei
{
                    ...
                    case 'ir_a_carpeta':
                        ...
                        if (isset($this->_path_absoluto)) {
                            if (strpos(realpath($seleccion), realpath($this->_path_absoluto)) !== 0) {
                                throw new toba_error_seguridad('El path es inválido');
                            }
                        }
    ...
    protected function validacion_basica_nombre($nombre)
    {
        ...
        foreach ($this->_caracteres_invalidos_nombres as $char) {
            if (strpos($nombre, $char) !== false) {
                ...
                $invs = implode(', ', $invs);
                toba_logger::instancia()->error("El nombre $nombre posee caracteres inválidos ($invs)");
     ...
     */
    public function set_path_absoluto($dir)
    {
     ...
     * @param boolean $solo
     */
    public function set_solo_carpetas($solo)
    {
     ...
     * @param boolean $crear
     */
    public function set_crear_carpetas($crear)
    {
     ...
     * @param boolean $crear
     */
    public function set_crear_archivos($crear)
    {
     ...
     */
    public function set_extensiones_validas($extensiones)
    {
     ...
     */
    public function set_permitir_espacios_en_nombres($permitir = true)
    {
