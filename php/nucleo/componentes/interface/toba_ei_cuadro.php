...
class toba_ei_cuadro extends toba_ei
{
	...
	protected static $_mostrar_excel_sin_cortes = false;    // Especifica si se tiene que dar la opci�n de renderizar como excel sin cortes
	...
	function eliminar_columnas($columnas)
	{
		foreach($columnas as $clave) {
            if (isset($this->_info_cuadro_columna_indices[$clave])) {
                ...
                $this->procesar_definicion_columnas();		//Se re ejecuta por eliminaci�n para actualizar $this->_info_cuadro_columna_indices
            }
		}
	}
	...
	*/
			...
			foreach ($clase->getMethods() as $metodo) {
				...
				if(! is_null($id)) {
					if(isset($this->_sum_usuario[$id])){
						toba_logger::instancia()->error("Las funciones de sumarizaci�n deben tener IDs �nicos. El id '$id' ya existe");
						throw new toba_error_def('La funci�n de sumarizaci�n no es correcta, revise el log');
					}
	...
	function set_manejador_salida ($tipo_salida, $clase)
	{
		if (is_null($tipo_salida)){
			throw new toba_error_seguridad('Se debe indicar un tipo de salida v�lido');
		}
		if (is_null($clase)) {
			throw new toba_error_seguridad('Se debe indicar un nombre de clase v�lido para el tipo de salida seleccionado ');
		}
	...
	*/
	...
	private function validar_y_separar_clave(&$klave)
	{
		if (! isset($this->_memoria['claves_enviadas']) || ! in_array($klave, $this->_memoria['claves_enviadas'])) {
			...
			throw new toba_error_seguridad(' La clave seleccionada no es v�lida');
		}
	...
	*/
	...
	function agregar_corte_control($corte)
	{
		...
		if (! isset($corte['columnas_descripcion'])) {
			throw new toba_error_def('El corte de control no posee la/s columna/s para la descripci�n');
		}
	...
	*/
	...
	/**
	 * Devuelve la cantidad de paginas que posee el cuadro de acuerdo a la los datos y el tama�o de pagina
	 * @return integer
	 */
	...
	private function get_sentido_ordenamiento($valor = 'asc')
	{
		if ($valor == 'asc') {
			...
			toba_logger::instancia()->error('Sentido de ordenamiento inv�lido: '. $valor);
			throw new toba_error_def('Sentido de ordenamiento inv�lido');
		}
		return $sentido;
	}
	...
	*/
	...
	function set_datos($datos)
	{
		...
		if (!is_array($this->datos)) {
			toba_logger::instancia()->error( $this->get_txt() .
					" El par�metro para cargar el cuadro posee un formato incorrecto:" .
						"Se esperaba un arreglo de dos dimensiones con formato recordset.");
			
			throw new toba_error_def(' El par�metro para cargar el cuadro posee un formato incorrecto, revise el log');
		}
	...
	*/
	...
	protected function generar_salida($tipo, $objeto_toba_salida = null)
	{
		if($tipo!="html" && $tipo!="impresion_html" && $tipo!="pdf" && $tipo!='excel' && $tipo!='xml'){
			toba_logger::instancia()->error("El tipo de salida '$tipo' es inv�lido");
			throw new toba_error_seguridad("El tipo de salida indicado es inv�lido, revise el log");
		}
	...
	*/
	...
	//############################################################################################
	/**
	 * Esta funci�n debe ser utilizada desde los archivos de customizaci�n
