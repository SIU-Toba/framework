...
class toba_ei_cuadro_salida_html extends toba_ei_cuadro_salida
{
    ...
    public function generar_botones($clase = '')
    {
     ...
     */
    public function html_mensaje_cuadro_vacio($texto)
    {
        $this->_cuadro->resetear_claves_enviadas();
        $info_cuadro = $this->_cuadro->get_informacion_basica_cuadro();
    ...
    protected function html_botonera_selector()
    {
        ...
        echo toba_form::button_html(
            "{$objeto_js}_subir",
            toba_recurso::imagen_toba('nucleo/orden_subir.gif', true),
            "onclick='{$objeto_js}.subir_fila_selector();'",
            0,
            '<',
            'Sube una posición la fila seleccionada'
        );
        echo toba_form::button_html(
            "{$objeto_js}_bajar",
            toba_recurso::imagen_toba('nucleo/orden_bajar.gif', true),
            "onclick='{$objeto_js}.bajar_fila_selector();' ",
            0,
            '>',
            'Baja una posición la fila seleccionada'
        );
        echo '</div>';
    }
    ...
    public function inicio_datos_corte($id_unico, $es_tabla_gral, $nivel_nodo)
    {
        echo toba::output()->get('CuadroSalidaHtml')->getInicioCorte($id_unico, $es_tabla_gral, $nivel_nodo);
    }

    public function fin_datos_corte($es_tabla_gral)
    {
     ...
     * @ignore
     */
    public function html_cc_inicio_nivel()
    {
    ...
    */
    public function html_cabecera_corte_control(&$nodo, $id_unico = null)
    {
    ...
    */
     ...
     * @ignore
     */
    public function html_pie_corte_control(&$nodo, $es_ultimo)
    {
        if ($this->_cuadro->get_cortes_modo() == apex_cuadro_cc_tabular) {				//MODO TABULAR
            ...
            if (isset($nodo['acumulador'])) {
                ...
                $this->html_cuadro_totales_columnas(
                    $nodo['acumulador'],
     ...
     */
    public function html_cabecera_pie($indice, $nodo, $total_columnas)
    {
        $nivel_css = $this->get_nivel_css($nodo['profundidad']);
     ...
     */
    public function html_pie_pie($nodo, $total_columnas, $es_ultimo)
    {
        $nivel_css = $this->get_nivel_css($nodo['profundidad']);
        $css_pie = 'ei-cuadro-cc-pie-nivel-' . $nivel_css;
     ...
     * @param integer $total_columnas
     */
    public function html_sumarizacion_usuario($nodo, $total_columnas)
    {
     ...
     */
    public function html_cuadro(&$filas, $totales = 0, $nodo = null)
    {
    ...
    public function get_estilo_seleccion($clave_fila)
    {
    ...
    public function generar_layout_fila($columnas, $datos, $id_fila, $clave_fila, $evt_multiples, $objeto_js, $estilo_fila, $formateo)
    {
    ...
    * @ignore
    */
    public function html_acumulador_usuario()
    {
        $sumarizacion = $this->_cuadro->calcular_totales_sumarizacion_usuario();
        $total_columnas = $this->_cuadro->get_cantidad_columnas_total();
        if (! empty($sumarizacion)) {
            $css = 'cuadro-cc-sum-nivel-1';
            echo toba::output()->get('CuadroSalidaHtml')->getSumarizacion($sumarizacion, null, 300, $css, null, "<tr><td colspan='$total_columnas'>\n", "</td></tr>\n");
        }
    }
