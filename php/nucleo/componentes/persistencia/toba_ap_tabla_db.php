...
abstract class toba_ap_tabla_db implements toba_ap_tabla
{
	...
	/**
	 * Shorcut a toba::logger()->debug incluyendo infomaci�n b�sica del componente
	 */
	...
	/**
	 * Carga una columna separada del proceso com�n de carga
	 ...
	 * @param array $col_resultado Columnas del registro resultante que se tomar�n para rellenar la tabla
	 ...
	 * necesitan mantenerse junto al conjunto de datos.
	 *
	 * @param string $m�todo M�todo que obtiene los datos.
	...
	/**
	 * Carga una columna separada del proceso com�n de carga
	 * Se brinda un Datos Tabla  que carga una o m�s columnas denominadas como 'externas'
	 * Una columna externa no participa en la sincronizaci�n posterior, pero por necesidades casi siempre est�ticas
	 * necesitan mantenerse junto al conjunto de datos.
	 *
	...
	/**
	 * Activa/Desactiva el uso autom�tico del trim sobre datos en el insert o update
	 * @param boolean $usar
	 */
	...
	/**
	 * Activa/Desactiva un mecanismo de chequeo de concurrencia en la edici�n
	 */
	...
	{
		// si no est� seteado $col['tabla'] entonces no puede ser un ap
	...
	function cargar_con_sql($sql, $anexar_datos=false, $usar_cursores=false)
	{
		...
		try{
			...
			$mensaje = "Error cargando la tabla <b>$this->_tabla</b>, a continuaci�n el mensaje de la base:<br>".$mensaje;
	...
	function sincronizar($filas=array())
	{
		...
		try{
			...
			if($this->_utilizar_transaccion) {
				toba::logger()->info("Abortando transacci�n en {$this->_fuente}", 'toba');
	...
	/**
	 * Ventana para inclu�r validaciones (disparar una excepcion) o disparar procesos previo a sincronizar con la base de datos
	...
	/**
	 * Ventana para inclu�r validaciones (disparar una excepcion) o disparar procesos antes de terminar de sincronizar con la base de datos
	...
	/**
	 * Ventana para manejar la p�rdida de sincronizaci�n con la tabla en la base de datos
	 * El escenario es que ejecuto un update/delete usando los valores de las columnas originales y no arrojo resultados, con lo que se asume que alguien m�s modifico el registro en el medio
	 * La transacci�n con la bd a�n no se termin� (si es que esta definida)
	 *
	 * @param integer $id_fila Id. de fila de la tabla en la cual se encontr� el problema
	 * @param string $sql_origen Sentencia que se intento ejecutar
	 * @ventana
	 */
	function evt__perdida_sincronizacion($id_fila, $sql_origen)
	{
		$mensaje_usuario = "Error de concurrencia en la edici�n de los datos.<br><br>".
							"Mientras Ud. editaba esta informaci�n, la misma fue modificada por alguien m�s. ".
							"Para garantizar consistencia s�lo podr� guardar cambios luego de reiniciar la edici�n.<br>";

		//--Hace una consulta SQL contra la tabla para averiguar puntualmente cu�l fue el cambio que llevo a esta situaci�n
		$columnas = array();
		...
		if ($fila_base === false) {
			...
			foreach ($columnas as $col) {
				...
				if ($modificado) {
					...
					$diff .= "<li>$col: ten�a el valor $anterior y ahora tiene $actual </li>";
				}
			}
			$diff .= '</ul>';
		}
	...
	/**
	 * Ventana de extensi�n previo a la inserci�n de un registro durante una sincronizaci�n con la base
	 * @param mixed $id_registro Clave interna del registro
	 * @ventana
	 */
	...
	/**
	 * Ventana de extensi�n posterior a la inserci�n de un registro durante una sincronizaci�n con la base
	 * @param mixed $id_registro Clave interna del registro
	 * @ventana
	 */
	...
	/**
	 * Ventana de extensi�n previo a la actualizaci�n de un registro durante una sincronizaci�n con la base
	 * @param mixed $id_registro Clave interna del registro
	 * @ventana
	 */
	...
	/**
	 * Ventana de extensi�n posterior a la actualizaci�n de un registro durante una sincronizaci�n con la base
	 * @param mixed $id_registro Clave interna del registro
	 * @ventana
	 */
	...
	/**
	 * Ventana de extensi�n previa al borrado de un registro durante una sincronizaci�n con la base
	 * @param mixed $id_registro Clave interna del registro
	 * @ventana
	 */
	...
	/**
	 * Ventana de extensi�n posterior al borrado de un registro durante una sincronizaci�n con la base
	 * @param mixed $id_registro Clave interna del registro
	 * @ventana
	 */
	...
	function get_sql_de_carga($campos)
	{
		if (isset($this->_sql_carga)) {
			...
			throw new toba_error_def("AP-TABLA Db: La tabla no ha sido cargada en este pedido de p�gina");
		}
	}
	...
	function generar_sql_where_registro($id_registro)
	{
		...
		if (! $this->_lock_optimista) {
			//Sin lock optimista arma el where s�lo con las claves
			foreach($this->_clave as $clave){
	...
	protected function usar_metodo_sql_fila($fila, $parametros)
	{
		...
		if (!$datos && $es_obligatoria) {
			...
			$this->log("AP_TABLA: [{$this->_tabla}]:\n ERROR en la carga de la columna externa.", 'error');
			throw new toba_error_def('AP_TABLA: ERROR en la carga de una columna externa. Revise el log');
		}
		return $datos;
	}
	...
	protected function usar_metodo_dao($param_dao, $parametros, $es_carga_inicial = false)
	{
		...
		if (isset($parametros['clase']) && isset($parametros['include'])) {
			...
			if (method_exists($this, $nombre_metodo)) {
				...
				$this->log(' ERROR en la carga de una columna externa. El m�todo: '. $nombre_metodo .' no esta definido', 'error');
				throw new toba_error_def('AP_TABLA_DB: ERROR en la carga de una columna externa. M�todo no definido, revise el log');
			}
		}
		return $datos;
	}
	...
	protected function usar_metodo_dt($param_dt, $parametros, $es_carga_inicial = false)
	{
		...
		$dt = toba_constructor::get_runtime($id, 'toba_datos_tabla');
		 if (! method_exists($dt, $nombre_metodo)) {
			$clase = get_class($dt);
			$this->log("ERROR en la carga de una columna externa. No existe el m�todo '$nombre_metodo' de la clase '$clase'", 'error');
			throw new toba_error_def('AP_TABLA_DB: ERROR en la carga de una columna externa. M�todo no definido, revise el log');
		}
	...
	protected function usar_clase_consulta_php($param_clase, $parametros, $es_carga_inicial = false)
	{
		...
		if (method_exists($obj, $nombre_metodo)) {
			...
			$this->log(' ERROR en la carga de una columna externa. El m�todo: '. $nombre_metodo .' no esta definido en la clase de consulta '. $parametros['clase'], 'error');
			throw new toba_error_def('AP_TABLA_DB: ERROR en la carga de una columna externa. M�todo no definido, revise el log');
		}
		return $datos;
	}
	...
	protected function adjuntar_campos_externos_masivo($datos, $externos, $parametros)
	{
		...
		if (empty($campos_externos) && $es_obligatoria) {
			$this->log('El m�todo de carga masiva no devuelve los campos clave, no se puede adjuntar los datos externos', 'error');
			throw new toba_error_def('AP_TABLA_DB: ERROR El m�todo de carga no devuelve los campos clave, no se puede adjuntar los datos externos');
		}
	...
	protected function verificar_existencia_valores($fila, $parametros, $evento = null)
	{
			...
			foreach( $parametros['col_parametro'] as $col_llave ) {
				if (isset($evento) && isset($this->_secuencias[$col_llave])) {
					...
					throw new toba_error_def('AP_TABLA_DB: No puede actualizarse en l�nea un valor secuencia, revise el log');
				}
