...
/**
 * Mantiene un conjunto relacionado de @see toba_datos_tabla , brindando servicios para cargar y sincronizar esta relaci�n con alg�n medio de persistencia (general una BD relacional)
 * 
...
class toba_datos_relacion extends toba_componente 
{
	...
	/**
	 * Deshabilita la tabla o las tablas recibidas para la carga y la sincronizaci�n
	 */
	...
	/**
	 * Retorna las tablas de una relaci�n
	...
	/**
	 * Retorna las tablas que est�n habilitadas para la carga y la sincronizaci�n
	 * @return array de toba_datos_tabla
	 */
	function get_tablas_activas()
	{
		...
		/**
	 * Retorna relaciones de las tablas que est�n habilitadas para la carga y la sincronizaci�n
	 * @return array de toba_datos_tabla
	 */
	...
	*/
	...
	/**
	 * Carga en el nodo xml los datos cargados en el DR. Funciona s�lo para relaciones que se modelan como un �rbol, no grafos.
	 * @param SimpleXMLElement $xml Es el nodo XML donde se van a cargar todos los datos
	 */
	
	function get_xml($xml){

		// Controla que haya una �nica tabla raiz
		if(count($this->_tablas_raiz) != 1)
			throw new toba_error_def('El datos_relacion no posee una �nica tabla raiz.');
	 ...
	 * Es un m�todo recursivo con la siguiente estructura:
	 * a) Setea el cursor en el registro dado de la tabla dada
	 * b) agrega los datos del registro en el que se est� parado como atributos del nodo xml recibido
	 * c) Para cada tabla hija agrega un nodo al nodo dado.
	 * d) Para cada registro de cada tabla hija, agrega un nodo al nodo creado en c) y llama recursivamente a la funci�n
	 *
	 * @param SimpleXMLElement $xml es el nodo donde se va a agregar la informaci�n
	...
	{
		// Setea el cursor de la tabla (esto adem�s est� seteando un cursor en las tablas hijas
		...
		// Recupera las tablas hijas de la tabla que se est� recorriendo
		...
		foreach($tablas_hijas as $tabla_hija){
			...
			// Para cada registro de las tablas hijas seleccionado seg�n el cursor, agrego un nodo y llamo recursivamente
	...
	protected function get_tablas_hijas($tabla)
	{
		...
		if(!$id_objeto_padre){
			toba_logger::instancia()->error("No se puede obtener el conjunto de tablas hija de la tabla $tabla, que no pertenece a la relaci�n.");
			throw new toba_error_def('La tabla no pertenece a la relaci�n, revise el log');
		}
