...
class toba_datos_relacion extends toba_componente
{
    ...
    protected static $debug_pasadas;				//Mantiene la cantidad de pasadas para generar ids unicos en js
    ...
    private function crear_relaciones()
    {
        if (count($this->_info_relaciones) > 0) {
            for ($a = 0;$a < count($this->_info_relaciones);$a++) {
                ...
                $this->_relaciones[$id_relacion] = new toba_relacion_entre_tablas(
                    $this->_dependencias[ $id_padre ],
                    $columnas_padre,
                    $id_padre,
     ...
     */
    public function desactivar_tablas($tablas)
    {
        if (is_array($tablas)) {
     ...
     */
    public function dump_contenido($etiqueta = null)
    {
     ...
     */
    public function dump_esquema($titulo = null)
    {
     ...
     */
    public function orden_sincronizacion()
    {
     ...
     */
    public static function grafo_relaciones($tablas, $relaciones)
    {
     ...
     * @return array
     */
    public function get_lista_tablas()
    {
     ...
     * @return array de toba_datos_tabla
     */
    public function get_tablas_activas()
    {
     ...
     * @return array de toba_datos_tabla
     */
    public function get_relaciones_activas()
    {
        if (empty($this->_tablas_inactivas)) {
     ...
     */
    public function resetear_cursores()
    {
        foreach ($this->_dependencias as $dependencia) {
     ...
     */
    public function set_cargado($cargado)
    {
    ...
    */
     ...
     * Sincroniza los cambios con el medio de persistencia
     */
    public function sincronizar_filas($filas_tablas)
    {
        foreach ($this->_dependencias as $id => $dependencia) {
     ...
     * @return array
     */
    public function get_tablas_raiz()
    {
    ...
    public function get_columnas_tabla_padre($datos)
    {
        $resultado = array();
    ...
    public function get_columnas_tabla_hija($datos)
    {
        $resultado = array();
        foreach ($this->_info_columnas_asoc_rel as $info_rel) {
