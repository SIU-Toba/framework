...
class toba_datos_tabla extends toba_componente
{
            ...
            foreach ($this->_info_externas as $externa) {
                ...
                if ($externa['sql'] != '') {
                    ...
                    $this->persistidor()->activar_proceso_carga_externa_sql(
                        ...
                        $parametros,
                        $resultados,
                        $externa['sincro_continua'],
                        $externa['dato_estricto']
                    );
                } else {
    ...
    protected function definir_metodo_carga_dao($externa, $resultados, $parametros)
    {
        ...
        if (isset($externa['carga_consulta_php']) && !is_null($externa['carga_consulta_php'])) {
            $this->persistidor()->activar_proceso_carga_externa_consulta_php(
                $externa['metodo'],
                $externa['carga_consulta_php'],
                $parametros,
                $resultados,
                $externa['sincro_continua'],
                $externa['dato_estricto'],
                $externa['permite_carga_masiva'],
                $externa['metodo_masivo']
            );
    ...
    public function set_definicion_columna($columna, $propiedad, $valor)
    {
     ...
     */
    public function reservar_id_fila()
    {
     ...
     */
    public function get_proximo_id()
    {
     ...
     * @ignore
     */
    public function agregar_relacion_con_padre($relacion, $id_padre)
    {
     ...
     * @ignore
     */
    public function get_relaciones_con_padres()
    {
     ...
     * @ignore
     */
    public function get_relacion_con_padre($id_tabla_padre)
    {
     ...
     * @ignore
     */
    public function agregar_relacion_con_hijo($relacion, $id_hijo)
    {
    ...
    */
     ...
     * @ignore
     */
    public function notificar_padres_carga($reg_hijos = null)
    {
     ...
     * @ignore
     */
    public function notificar_hijos_sincronizacion($filas = array())
    {
     ...
     * @param boolean $unico
     */
    public function set_es_unico_registro($unico)
    {
     ...
     */
    public function restaurar_cursor()
    {
     ...
     */
    public function resetear_cursor()
    {
        unset($this->_cursor);
     ...
     */
    public function get_filas($condiciones = null, $usar_id_fila = false, $usar_cursores = true)
    {
        $datos = array();
        $a = 0;
     ...
     */
    public function get_id_filas($usar_cursores = true)
    {
        $coincidencias = array();
     ...
     * @todo Se podría optimizar este método para no recaer en tantos recorridos
     */
    public function get_id_filas_filtradas_por_cursor($incluir_eliminados = false)
    {
        if ($this->hay_cursor()) {
     ...
     */
    public function get_id_padres($ids_propios, $tabla_padre)
    {
        $salida = array();
    ...
    */
    public function get_id_fila_padre($tabla_padre, $id_fila)
    {
     ...
     */
    public function get_id_fila_condicion($condiciones = null, $usar_cursores = true)
    {
     ...
     */
    public function get_columna($columna)
    {
     ...
     * @return integer
     */
    public function get_cantidad_filas($usar_cursores = true)
    {
     ...
     * @param boolean $usar_cursores Este conjunto de filas es afectado por la presencia de cursores en las tablas padres
     */
    public function existe_fila_condicion($condiciones, $usar_cursores = true)
    {
     ...
     */
    public function nueva_fila($fila = array(), $ids_padres = null, $id_nuevo = null)
    {
        $this->notificar_contenedor("ins", $fila);
        //Saco el campo que indica la posicion del registro
        if (isset($fila[apex_datos_clave_fila])) {
            unset($fila[apex_datos_clave_fila]);
        }
        if (isset($fila[apex_ei_analisis_fila])) {
     ...
     */
    public function modificar_fila($id, $fila, $nuevos_padres = null)
    {
        $id = $this->normalizar_id($id);
        if (!$this->existe_fila($id)) {
     ...
     * 				El resto de los padres sigue con la asociación anterior
     */
    public function cambiar_padre_fila($id_fila, $nuevos_padres)
    {
     ...
     */
    public function eliminar_filas($con_cursores = true)
    {
     ...
     */
    public function set_columna_valor($columna, $valor, $con_cursores = false)
    {
        if (! isset($this->_columnas[$columna])) {
            toba_logger::instancia()->error("La columna '$columna' no es válida");
     ...
     */
    public function set_blob($columna, $blob, $id_fila = null)
    {
     ...
     * @return resource
     */
    public function get_blob($columna, $id_fila = null)
    {
        if (! isset($id_fila)) {
            if ($this->get_cantidad_filas() == 0) {
                return null;
     ...
     * @ignore
     */
    public function _get_blob_transaccion($id_registro, $col)
    {
    ...
    protected function validar_columnas_en_fila($columnas, $fila, $id)
    {
        foreach (array_keys($this->_datos) as $id_fila) {					  //Recorro todos los datos cargados
            if (! is_null($id) && ($id_fila == $id)) {
                continue;
            }						//Si es la misma fila no se procesa
     ...
     */
    public function validar($filas = array())
    {
     ...
     * @ventana
     */
    public function evt__validar_fila($fila)
    {
    }
    ...
    */
     ...
     */
    public function cargar($clave = array())
    {
     ...
     */
    public function anexar_datos($datos, $usar_cursores = true)
    {
     ...
     */
    public function sincronizar($usar_cursores = false)
    {
        if ($usar_cursores) {
     ...
     * @return integer Cantidad de registros modificados en el medio
     */
    public function sincronizar_filas($filas)
    {
        $this->validar($filas);
        $modif = $this->persistidor()->sincronizar($filas);
        $this->notificar_fin_sincronizacion($filas);
     ...
     * @ignore
     */
    public function notificar_fin_sincronizacion($filas = array())
    {
     ...
     */
    public function get_tabla_extendida()
    {
    ...
    public function get_schema_ext()
    {
    ...
    *
    */
    public function forzar_insercion($usar_cursores = false, $filas = null)
    {
        if ($usar_cursores) {
     ...
     * @return boolean
     */
    public function hay_cambios_fila($id_fila)
    {
     ...
     */
    public function get_cambios_fila($id_fila, $datos_ap = array())
    {
        $diferencias = array();
     ...
     * @return boolean
     */
    public function es_campo_modificado($campo, $id_viejos, $datos_nuevos)
    {
        if (! isset($this->_columnas[$campo])) {
            return false;
        }
