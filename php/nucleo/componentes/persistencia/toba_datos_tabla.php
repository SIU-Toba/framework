...
class toba_datos_tabla extends toba_componente 
{
	...
	*/
	...
	/**
	 * Indica que la tabla maneja un �nico registro en memoria, habilitando la api get/set
	 * @param boolean $unico
	 */
	...
	function set_cursor($id)
	{
		...
		if( $this->existe_fila($id) ){
			...
			toba_logger::instancia()->error($this->get_txt() . "La fila '$id' no es v�lida");
			throw new toba_error_def('La fila solicitada no es v�lida');
		}
	}	
	...
	*/
			...
			foreach($keys_c as $pos) {
				...
				foreach( array_keys($condiciones) as $campo) {
					...
					if( !isset($this->_columnas[$columna]) ){
						...
						throw new toba_error_def('P�rametro de filtrado incorrecto, revise el log');
					}
	...
	function cambiar_padre_fila($id_fila, $nuevos_padres)
	{
		...
		foreach ($nuevos_padres as $tabla_padre => $id_padre) {
			if (!isset($this->_relaciones_con_padres[$tabla_padre])) {
				$mensaje = ' CAMBIAR PADRE. No existe una relaci�n padre';
	...
	function set_fila_columna_valor($id, $columna, $valor)
	{
		...
		if( $this->existe_fila($id) ){
			if( isset($this->_columnas[$columna]) ){
				...
				toba_logger::instancia()->error("La columna '$columna' no es v�lida");
				throw new toba_error_def('Columna no v�lida, revise el log');
			}
		}else{
			toba_logger::instancia()->error("La fila '$id' no es v�lida");
			throw new toba_error_def('La fila no es v�lida, revise el log');
		}
	}
	...
	function set_columna_valor($columna, $valor, $con_cursores=false)
	{
		if(! isset($this->_columnas[$columna]) ) { 
			toba_logger::instancia()->error("La columna '$columna' no es v�lida");
			throw new toba_error_def('La columna no es v�lida, revise el log');
		}
		foreach($this->get_id_filas($con_cursores) as $fila) {
...
*/
	...
	protected function control_tope_minimo_filas()
	{
		if ($this->_tope_min_filas != 0 && $this->get_cantidad_filas() < $this->_tope_min_filas) {
			toba_logger::instancia()->error("La tabla <em>{$this->_id_en_controlador}</em> requiere ingresar al menos {$this->_tope_min_filas} registro/s (se encontraron s�lo {$this->get_cantidad_filas()}).");
			throw new toba_error_validacion('El n�mero de filas enviadas no es adecuado');
		}
	}
	...
	protected function control_tope_maximo_filas($cantidad)
	{
		if (($this->_tope_max_filas != 0) && ($cantidad > $this->_tope_max_filas)) {
			toba_logger::instancia()->error("No est� permitido ingresar m�s de {$this->_tope_max_filas} registros en la tabla <em>{$this->_id_en_controlador}</em> (se encontraron $cantidad).");
			throw new toba_error_validacion('El n�mero de filas enviadas no es adecuado');
		}
	}
	...
	function persistidor()
	{
		if(!isset($this->_persistidor)){
			if($this->_info_estructura['ap']=='0'){
				...
				if( (trim($clase) == '' ) ){
					toba_logger::instancia()->error($this->get_txt() . 'Error en la definici�n, falta definir la subclase');
					throw new toba_error_def('Error de definici�n del persistidor, revise el log');
				}
			}else{
	...
	/**
	 * Sincroniza un conjunto de filas de la tabla en memoria con el medio f�sico a trav�z del administrador de persistencia.
	 *
	 * @return integer Cantidad de registros modificados en el medio
	 */
	...
	*/
	...
	/**
	 * Determina si los datos cargados en la tabla difieren de los datos existentes en la base al inicio de la transacci�n
	 * @return boolean
	 */	
	...
	/**
	 * Retorna verdadero si alg�n valor de la tabla cambio desde el inicio de la transacci�n
	 * @return boolean
	 */
