...
class objeto_datos_relacion extends objeto
{
	...
	private function crear_relaciones()
	{
		if(count($this->info_relaciones)>0){
			for($a=0;$a<count($this->info_relaciones);$a++)
			{
				...
				$this->relaciones[$id_relacion] = new relacion_entre_tablas(	$this->dependencias[ $id_padre ],
	...
	protected function dump_esquema_relaciones($nodo)
	{
		...
		foreach ($nodo->getNeighbours() as $nodo_vecino) {
			...
			//Busco los relacion_entre_tablas correspondientes
	 ...
	 * @return array Arreglo de objeto_datos_tabla
	 */
	...
	//Devuelve una referencia a una tabla para trabajar con ella
	{
		if($this->existe_tabla($tabla)){
			...
			throw new excepcion_toba("El datos_tabla '$tabla' solicitado no existe.");
		}
	}
