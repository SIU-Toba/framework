...
class toba_cargador
{
    ...
    */
    public function get_metadatos_simples($componente, $tipo = null, $db = null)
    {
        $metadatos = array();
        if (!isset($tipo)) {
    ...
    */
    public function get_metadatos_simples_diff($componente, $tipo = null, $db = null)
    {
        $metadatos = array();
        if (!isset($tipo)) {
            $tipo = self::get_tipo($componente);
    ...
    */
    public function get_metadatos_extendidos($componente, $tipo = null, $db = null, $resumidos = false)
    {
        if (!isset($tipo)) {
            $tipo = self::get_tipo($componente);
        }
        ...
        if (isset($this->cache_metadatos_extendidos)) {			// CACHE de EXTENDIDOS no implementado!
            ...
            $estructura = call_user_func_array(
                array(	$clase_def,
                                                        $metodo_def ),
                array( $proyecto, $id )
            );
     ...
     */
    public function set_metadatos_extendidos($metadatos, $componente)
    {
     ...
     */
    public function get_metadatos_redefinidos($componente)
    {
        $proyecto = $componente['proyecto'];
        $id = $componente['componente'];
        $clave_ser = $proyecto.'||'.$id;
     ...
     * @return string
     */
    public static function get_tipo($componente)
    {
        $db = toba::instancia()->get_db();
    ...
    public static function cargar_clase_archivo($pm, $clase_archivo, $id_proyecto)
    {
        $path = '';
        if (! is_null($pm)) {
     ...
     * @ignore
     */
    public function crear_cache_simple($proyecto, $db = null)
    {
        ...
        foreach (toba_db_tablas_componente::get_lista() as $tabla) {
            ...
            $sql = 'SELECT ' . implode(', ', $definicion['columnas']) .
                    ...
                    " ORDER BY {$definicion['dump_order_by']} ;\n";
            //Agrego la tabla
            $this->cache_metadatos_simples->agregar_tabla(
                $tabla,
                $sql,
     ...
     * @ignore
     */
    public function generar_cache_extendido($proyecto, $db = null)
    {
    }
}
