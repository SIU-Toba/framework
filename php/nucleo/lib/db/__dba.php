...
class dba
{
	...
	private static $bases_registradas;
	...
	*/
	...
	private static function get_info_db($nombre)
	{
		if (!isset(self::$info_bases[$nombre])) {
			...
			if ($nombre == 'instancia') {
				self::$info_bases[$nombre] = self::get_parametros_base($nombre);
			} else {
				$sql = "SELECT 	*, 
				...
				if ($rs[0]['link_instancia'] == 1) {
					...
					$datos = array_merge($datos, self::get_parametros_base($id_instancia));
				}
	...
	static function get_parametros_base($nombre)
	{
		self::get_info_bases();
		$nombre_base = ($nombre == 'instancia') ? apex_pa_instancia : $nombre;
		if (!isset(self::$bases_registradas[$nombre_base]))
			throw new excepcion_toba("La entrada '$nombre_base' no esta definida en el archivo de bases: '" . self::path_archivo_bases . "'" );
		$datos['profile'] = self::$bases_registradas[$nombre_base][apex_db_profile];
		$datos['motor'] =  self::$bases_registradas[$nombre_base][apex_db_motor];
		$datos['usuario'] = self::$bases_registradas[$nombre_base][apex_db_usuario];
		$datos['clave'] = self::$bases_registradas[$nombre_base][apex_db_clave];
		$datos['base'] = self::$bases_registradas[$nombre_base][apex_db_base];
		$datos['fuente_datos'] = $nombre;
		return $datos;
	}
	
	static function get_info_bases()
	{
		if (!isset(self::$bases_registradas)) { 
