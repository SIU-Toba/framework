...
class toba_vista_araireportes
{
     ...
     */
    public function reset_parametros()
    {
     ...
     * @return string
     */
    public function get_nombre_archivo_generado()
    {
     ...
     */
    public function set_nombre_archivo($nombre)
    {
     ...
     */
    public function set_tipo_descarga($tipo)
    {
     ...
     */
    public function agregar_metareporte($reporte)
    {
     ...
     * @return boolean
     */
    public function hay_metareportes()
    {
     ...
     * @param $path String
     */
    public function set_path_reporte($path)
    {
        $this->uri = $path;
     ...
     */
    public function set_xml($xml, $xpath_data_search = null)
    {
        ...
        $this->archivo_temp_path = $nombre;

        $this->xpath_data = $xpath_data_search;
        $this->formato_datos = 'xml';
     ...
     * @param string $xpath_data_search XPath al nodo que contiene los datos. Opcional
     */
    public function set_archivo_xml($ruta_xml, $xpath_data_search = null)
    {
        $this->archivo_temp_path = toba_manejador_archivos::nombre_valido($ruta_xml);
        $this->xpath_data = $xpath_data_search;
        $this->formato_datos = 'xml';
        if (is_null($xpath_data_search)) {		//Si esto no viene no puedo crear un JRXMLDataSource, paso a modo archivo
            $this->modo_archivo = true;
    ...
    public function generar_salida()
    {
        ...
        if (($this->modo_archivo) &&  ($this->limpiar_modo_archivo)) {
            if (file_exists($this->archivo_temp_path)) {
                unlink($this->archivo_temp_path);
            }
        }
    }
     ...
     */
    public function crear_recursos_temporales()
    {
        //Si el conjunto de datos viene de un archivo comun
        if (isset($this->archivo_temp_path)) {
            $url = 'archivos';
     ...
     */
    public function eliminar_recursos_temporales()
    {
    }
