...
class toba_editor
{
	...
	*/
	...
	static function get_perfiles_funcionales_previsualizacion()
	{
		...
		if(isset($param_prev['grupo_acceso'])) {
			...
			throw new toba_error('No estan definidos los perfiles de acceso a la previsualizaci�n. Desde toba_editor se pueden definir en la opci�n de Configuraci�n > Previsualizaci�n');
		}
	}
	...
	*/
	static function generar_zona_vinculos_item( $item, $accion, $enviar_div_wrapper = true )
	{
		...
		$html_ayuda_ajax = toba_recurso::ayuda(null, 'Activar/Desactivar navegaci�n interna de la operaci�n via AJAX');
		...
		if (function_exists('memory_get_peak_usage')) {
			...
			echo toba_recurso::imagen_toba('memory.png', true, 16, 16, 'Pico m�ximo de memoria que ha consumido el script actual');
		...
		if ($fuente) {
			try {
				...
				echo "<a href='$link_analizador_sql' target='logger'>".toba_recurso::imagen_toba('objetos/datos_relacion.gif', true, 16, 16, 'Ver detalles de las consultas y comandos ejecutados en este pedido de p�gina').
					count($info_db). " ($rol)</a>";
		...
		echo "<a href='$link_archivos' target='logger'>".toba_recurso::imagen_toba('nucleo/php.gif', true, 16, 16, 'Ver detalle de archivos .php del proyecto incluidos en este pedido de p�gina').
			' '.count($archivos)." arch. (".file_size($total,0).')</a>';

		//Session
		$tamano = file_size(strlen(serialize($_SESSION)), 0);
		echo toba_recurso::imagen_toba('sesion.png', true, 16, 16, 'Tama�o de la sesi�n')." $tamano  ";
		echo "</span>";
		...
		if ($hay_limitado) {
			...
			$js = "title='Cambia temporalmente el usuario de conexi�n a la base' onchange=\"location.href = toba_prefijo_vinculo + '&usuario_conexion=' + this.value\"";
		...
		$js = "title='Cambia temporalmente el skin de la aplicaci�n' onchange=\"location.href = toba_prefijo_vinculo + '&skin=' + this.value\"";
	...
	*/
	...
	static function get_vinculos_item( $item, $accion )
	{
		...
		$vinculo['tip'] = 'Ver composicion de la operaci�n en el editor.';
		$vinculos[] = $vinculo;
