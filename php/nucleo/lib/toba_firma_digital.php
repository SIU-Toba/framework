...
{
    public static function certificado_decodificar($certificado)
    {
    ...
    public static function certificado_get_fingerprint($certificado)
    {
        return sha1(self::certificado_decodificar($certificado));
    }

    public static function certificado_get_serial_number($certificado)
    {
        $data = openssl_x509_parse($certificado, false);
        return strtoupper(self::dec2hex($data['serialNumber']));
    }

    public static function certificado_validar_revocacion($certificado, $crl)
    {
        if (! file_exists($crl)) {
     ...
     */
    public static function certificado_validar_CA($certificado, $pem_ca)
    {
    ...
    public static function certificado_validar_expiracion($certificado)
    {
     ...
     */
    public static function pdf_add_attachments($archivo_pdf, $paths_attachments, $pagina = null)
    {
        $archivo_temp = toba::proyecto()->get_path_temp().'/'.md5(uniqid(time()));
        $stdout = null;
        $stderr = null;
     ...
     */
    public static function pdf_get_attachments($archivo_pdf, $patron = null)
    {
    ...
    private static function dec2hex($number)
    {
    ...
    private static function compare_openssl_date($in)
    {
        if (strlen($in) == 15) {
            $year = substr($in, 0, 4);
            $in = substr($in, 2);
        ...
        if ($today['minutes'] < $min) {
            return 1;
        }
        if ($today['minutes'] > $min) {
            return -1;
        }
        if ($today['seconds'] < $hour) {
            return 1;
        }
        if ($today['seconds'] > $hour) {
            return -1;
