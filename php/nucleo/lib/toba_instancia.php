...
class toba_instancia
{
	...
	static function get_datos_instancia($id_instancia)
	{
		...
		toba::config()->load();
		if (! is_file( $archivo ) ) {
			toba_logger::instancia()->error("INFO_INSTANCIA: No se encuentra definido el archivo de inicializaci�n de la INSTANCIA: '".self::get_id()."' ('$archivo')");
			throw new toba_error('INFO_INSTANCIA: No se encuentra definido el archivo de inicializaci�n de la INSTANCIA');
		}
	...
	function get_db()
	{
		if ( isset( $this->memoria['base'] ) ) {
			...
			toba_logger::error("INFO_INSTANCIA: El archivo de inicializaci�n de la INSTANCIA: '".self::$id."' no posee una BASE DEFINIDA");
			throw new toba_error('INFO_INSTANCIA: El archivo de inicializaci�n de la INSTANCIA no posee una BASE DEFINIDA');
		}
	}
	...
	/**
	 * Retorna la informaci�n cruda de un grupo de usuarios, tal como est� en la base de datos
	 * @see toba_usuario
	 */
	...
	function get_info_autenticacion($usuario)
	{
		try {
			...
			throw new toba_error('Error recuperando informaci�n');
		}
	}
	...
	*/
	...
	function get_id_sesion()
	{
		...
		$rs = $this->get_db()->consultar($sql);
		if(empty($rs)){
			throw new toba_error('No es posible recuperar el ID de la sesi�n.');
		}
		return $rs[0]['id'];
	}
