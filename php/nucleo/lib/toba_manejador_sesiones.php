...
class toba_manejador_sesiones
{
	...
	private function __construct()
	{
        ...
        private function iniciar_session_php()
        {
            ...
            if (session_id() != '') {
                    throw new toba_error('Ya existe una sesiÃ³n abierta, probablemente tenga activado session.auto_start = 1 en el php.ini');
            }
	...
	function login($id_usuario, $clave=null, $datos_iniciales=null)
	{
		...
		if ($this->contrasenia_vencida) {	//Si se vencio la contraseÃ±a del usuario redirecciono al item correspondiente
			$this->contrasenia_vencida = false;
			throw new  toba_error_login_contrasenia_vencida('La contraseÃ±a actual del usuario ha caducado');
		}
	...
	/**
	*	Entrada a un proyecto desde la operaciÃ³n de inicializacion de sesion
	*/
	...
	/**
	 * Delega la autenticaciÃ³n del proyecto a un objeto
	...
	*/
	...
	/**
	 * Retorna los perfiles funcionales activos en la sesiÃ³n actual
	 * @return array
	 */
	...
	function iniciar()
	{
		if( $this->existe_sesion_activa() ) {			//--> Hay una sesion para el proyecto actual
			...
			try {
				...
				toba::logger()->debug('PÃ©rdida de sesiÃ³n: '. $e->getMessage());
	...
	*/
	private function autenticar($id_usuario, $clave=null, $datos_iniciales=null)
	{
		...
		if ($this->invocar_metodo_usuario('es_usuario_bloqueado', array($id_usuario))) {
			throw new toba_error('El usuario se encuentra bloqueado. ContÃ¡ctese con el administrador');
		}
		...
		if ($this->get_autenticacion() != null) {
			...
			throw new toba_error_seguridad('No existe la autenticaciÃ³n propuesta');
		}
		if (!$estado) {
			$error = 'La combinaciÃ³n usuario/clave es incorrecta';
