...
class toba_manejador_sesiones
{
	...
	function cambio_usuario($actual, $nuevo, $datos_iniciales=null)
	{
		...
		if (empty($mapeo)) {
			throw new toba_error_seguridad('Es intentando acceder a un usuario no v�lido' );
		}
		//Si todo va bien.
	...
	private function control_finalizacion_sesion()
	{
		...
		if($ventana != 0) { // 0 implica desactivacion
			...
			if ( $tiempo_desconectado >= $ventana) {
				toba::notificacion()->agregar('Usted ha permanecido varios minutos sin interactuar
							con el servidor. Por razones de seguridad su sesi�n ha sido eliminada.
							Por favor vuelva a registrarse si desea continuar utilizando el sistema.
							Disculpe las molestias ocasionadas.');
				toba_logger::instancia()->error("Se excedi� la ventana temporal ($ventana m.)");
				throw new toba_error_autorizacion('Se excedi� la ventana temporal');
			}
		}
		...
		if($maximo != 0){ // 0 implica desactivacion
			...
			if ( $tiempo_total >= $maximo) {
				toba::notificacion()->agregar('Se ha superado el tiempo de sesi�n permitido
							Por favor vuelva a registrarse si desea continuar utilizando el sistema.
							Disculpe las molestias ocasionadas.');
				toba_logger::instancia()->error("Se excedi� el tiempo m�ximo de sesi�n ($maximo m.)");
				throw new toba_error_autorizacion('Se excedi� el tiempo m�ximo de sesi�n');
			}
		}
	...
	*/
	function abortar_sesion_proyecto($proyecto, $obs=null)
	{
		...
		toba::logger()->debug("Abortando la sesi�n '$id' del proyecto '$proyecto'.",'toba');
			...
			if (isset($cant_max_intentos)) {
				...
				if ( $supero_tope_intentos_en_ventana ) {
					if ($bloquear_usuario) {
						...
						throw new toba_error_autenticacion("$error. Ha superado el l�mite de inicios de sesi�n.");
					}elseif ($lanzar_excepcion) {
						toba_logger::instancia()->error("$error. Ha superado el l�mite de inicios de sesi�n.|$intentos");
						throw new toba_error_autenticacion_intentos("$error. Ha superado el l�mite de inicios de sesi�n.");
					} else {
            ...
            if ($handler_type !== false) {
                switch ($handler_type) {
                    ...
                    case 'memcache':
                        ...
                        break;
                    default:
                        throw new \toba_error_ini_sesion('El handler de sesi�n elegido no es v�lido');
                }
            }
            return new $handler();
        }
