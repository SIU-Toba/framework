<?php
/**
 * Brinda servicios de informaci�n sobre el proyecto actualmente cargado en el framework:
 *  - Informaci�n del archivo de configuraci�n proyecto.ini, cacheandolo en la memoria
 *  - Informaci�n de la definici�n b�sica en el editor (e.i. los metadatos)
 *
 * @package Centrales
 */
class toba_proyecto
{
	...
	/**
	 * Retorna el id del proyecto actualmente cargado en el pedido de p�gina
	 */
	...
	private function __construct($proyecto)
	{
		...
		if (!$this->memoria) {
			...
			//toba::logger()->debug('Inicializaci�n de TOBA_PROYECTO: ' . $this->id,'toba');
		}
	...
	/**
	 * Retorna el valor de un par�metro generico del proyecto (ej. descripcion) cacheado en la memoria
	 * @return toba_error si el parametro no se encuentra definido, sino el valor del par�metro
	 */
	function get_parametro($seccion, $parametro=null, $obligatorio=true)
	{
		...
		if( defined(self::prefijo_punto_acceso.$id) ){
			...
			if(\array_key_exists($id, $this->memoria)) {
				...
				if ($obligatorio) {
					toba_logger::instancia()->error("INFO_PROYECTO: El par�metro '$id' no se encuentra definido.");
					throw new toba_error('INFO_PROYECTO: El par�metro solicitado no se encuentra definido, revise el log.');
				} else {
					return null;
				}
			}
	...
	/**
	 * Retorna el n�mero de versi�n propio del proyecto
	 * @return toba_version
	 */
	 ...
	 * Si se pasa un path relativo como par�metro retorna el path absoluto del archivo en el directorio temporal
	 * @return array con claves 'path' (en el sist.arch.) y 'url' (URL navegable)
	 */
	 ...
	 * @return array RecordSet contienendo informaci�n de los items
	 */
		...
		if(!isset($this->indice_items_accesibles)) {
			...
			if ( toba::nucleo()->utilizar_metadatos_compilados( $this->id, 'compilados' ) ) {
				...
				if (! empty($grupos_acceso)) {
					...
					//-- Si no tiene grupo, busca aquellos items que son p�blicos
	...
	*/
	...
	/**
	 * Retorna la descripci�n asociada a un permiso global particular del proy. actual
	 */
	...
	function get_info_servicios_web_acc($id, $proyecto=null)
	{
		...
		if (empty($rs)) {
			toba_logger::instancia()->error("No se puede encontrar la definici�n del Servicio Web '$id' en el proyecto '$proyecto'");
			throw new toba_error('No se puede encontrar la definici�n del Servicio Web en el proyecto ');
		}
		return $rs;
	}
		...
		switch ($info['tipo_gadget']) {
			...
			case apex_tipo_gadget_interno:
							...
							if (! isset($info['subclase']) || ! isset($info['subclase_archivo'])) {
								throw new toba_error_def('La definici�n de subclase para el gadget esta incompleta');
							} else {
								$clase = $info['subclase'];
							...
							break;
			default:
							throw new toba_error_def('El tipo de gadget recuperado no es v�lido.');
		}
