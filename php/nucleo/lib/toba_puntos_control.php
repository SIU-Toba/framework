<?php
  ...
  class toba_puntos_control
  {
    ...
    {
      //Ejecuto el control
      $control->ejecutar($this->get_valores_punto($punto_control));  
    ...
    private function set_resultado($control)
    {
      ...
      if ($resultado === false || $resultado < 0) 
      {
        ...
        else if ( ($this->get_actua_como() == 'M' && $actua_como == 'A') ||  
                  ($this->get_actua_como() == 'A' && $actua_como == 'E') 
                )
          $data["actua_como"] = $actua_como;
      }
    }
    ...
    {
      /*$data =& $_SESSION['toba']['proyectos'][toba::proyecto()->get_id()];

      if (array_key_exists('puntos_control', $data))
        unset($data['puntos_control']);*/
    }
