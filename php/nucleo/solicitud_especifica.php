...
//--------------------------------------------------------------------------------------------
	function recibir_paquete()
	{
		if(isset($_POST[apex_soap_paquete])){
			$this->datos_recibidos = comunicador::desempaquetar( $_POST[apex_soap_paquete] );
			//Proceso Campos TOBA
			//Instancia que emitio el paquete
			if(isset($this->datos_recibidos[apex_soap_instancia])){
				$this->msg_instancia = $this->datos_recibidos[apex_soap_instancia];
				unset($this->datos_recibidos[apex_soap_instancia]);
			}else{
				$this->msg_instancia = "X";
			}
			//usuario de la instancia que envio el paquete
			if(isset($this->datos_recibidos[apex_soap_usuario])){
				$this->msg_usuario = $this->datos_recibidos[apex_soap_usuario];
				unset($this->datos_recibidos[apex_soap_usuario]);
			}else{
				$this->msg_usuario = "X";
			}
			//Momento en que se creo el paquete (en el emisor)
			if(isset($this->datos_recibidos[apex_soap_momento])){
				$this->msg_momento = $this->datos_recibidos[apex_soap_momento];
				unset($this->datos_recibidos[apex_soap_momento]);
			}else{
				$this->msg_momento = "";
			}
		}
...
//--------------------------------------------------------------------------------------------
	...
	function registrar()
	{
		...
		if($this->registrar_db){
			$cliente = $_SERVER["REMOTE_ADDR"];
			$sql = "INSERT INTO apex_solicitud_soap 
					(solicitud_soap, usuario, ip, instancia, instancia_usuario, paquete)
					VALUES ('$this->id','".apex_pa_usuario_anonimo."','".$cliente."','".$this->msg_instancia .
							"','".$this->msg_usuario."','".serialize($this->datos_recibidos)."');";
			if ($db["instancia"][apex_db_con]->Execute($sql) === false){
				monitor::evento("bug","SOLICITUD SOAP: No se pudo registrar la solicitud: " .
				$db["instancia"][apex_db_con]->ErrorMsg());
			}
		}
	}
