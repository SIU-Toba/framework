...
abstract class toba_solicitud
{
     ...
     */
    public function __construct($item, $usuario)
    {
     ...
     */
    public function set_cronometrar($cronometrar)
    {
     ...
     */
    public function cargar_objeto($clase, $posicion, $parametros = null)
    {
     ...
     */
    public function finalizar_objetos()
    {
     ...
     */
    public function guardar_cronometro()
    {
        if ($this->cronometrar) {
            if (! $this->registrada) {
                $this->registro_temprano();
     ...
     */
    abstract public function procesar();
     ...
     */
    public function registro_temprano()
    {
        if ($this->registrar_db || $this->cronometrar) {
            toba::instancia()->registrar_solicitud(
                $this->id,
                $this->info['basica']['item_proyecto'],
                $this->info['basica']['item'],
                $this->get_tipo()
            );
        ...
        if ($this->registrar_db || $this->cronometrar) {
            ...
            if (count($this->observaciones) > 0) {
                for ($i = 0;$i < count($this->observaciones);$i++) {
                    toba::instancia()->registrar_solicitud_observaciones(
                        $this->info['basica']['item_proyecto'],
                        $this->id,
     ...
     * @param type $tipo
     */
    public function observar($observacion, $tipo = null)
    {
        if (!isset($tipo)) {
     ...
     * @return boolean
     */
    public function es_item_publico()
    {
     ...
     * @return array
     */
    public function get_datos_item($prop = null)
    {
        if (isset($prop)) {
     ...
     */
    public function set_datos_item($prop = array())
    {
     ...
     * @return mixed
     */
    public function get_id_operacion()
    {
