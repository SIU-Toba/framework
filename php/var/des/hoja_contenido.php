...
*/
	...
	function contenido($datos_db, $definicion, $directivas)
	{
		...
		$this->graf_gen_tipo = trim($definicion["hoja_grafico"]);
...
*/
	...
	//Genera la estructura donde se especifica el formato de cada columna
	{
		...
		$this->formato_columnas[0]["estilo"]="dato-descripcion";
		...
		for ($a=0;$a<($this->dir_col_tipo[dato_valor]);$a++)
		{
			$temp[$a]["estilo"]="dato-valor-".$this->dir_col[($this->dir_col_posicion[dato_valor]+$a)]["estilo"];
	...
	//Dispara la generacion de HTML
	{
		...
		//Nivel 2 = bloques;
			//Nivel 1 = grupos;
				//Nivel 0 = tablas;
		return $this->$funcion_segun_CdC($this->arbol_nucleo);
 	}
	//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	//Genera el HTML de la cabecera
	{
		...
		for($a=0;$a<count($this->arbol_cabecera);$a++)
		{
			...
			$html .="<td class='columna-titulo-0'>
					<TABLE width='100%' cellpadding='0' cellspacing='0'>\n
						<TR>\n
							<TD class='columna-titulo-0-b' width='98%'>". $categoria ."</TD>\n
							<TD width='2%'><img src='".img_global."categ_x.gif' border='0'></TD>\n
						</TR>\n
					</TABLE>\n			
					</td>\n";
			...
			foreach($this->arbol_cabecera[$nivel] as $columna)
			{
				...
				$html .="<TD colspan='$colspan' class='columna-titulo-1'>" . $this->$formateador($nombre) . "</TD>\n";
				for($x=0;$x<$colspan;$x++)
				{
			...
			//Total en X.
			if ($this->dir_total_x) $html .="<TD>&nbsp;</TD>\n";
			//Popup grafico FILAS
		...
		//Total en X.
		if ($this->dir_total_x) $html .="<TD class='columna-titulo-0'>Total</TD>\n";
		//Popup grafico FILAS
		if ($this->graf_filas) $html .="<TD>&nbsp;</TD>\n";
		$html.="</TR>\n";		
	...
	//Genera las tablas.
	{
		...
		$html ="<TABLE width='$ancho' cellpadding='2' cellspacing='0' align='center'><tr><td>\n";
		$html.="<TABLE width='100%' cellpadding='2' cellspacing='0'>\n";		
		//Fila superior (Titulo, exportacio)
		$html.="<TR><TD colspan='". $this->columnas_total ."'>\n";		
		$html.="<TABLE width='100%' cellpadding='2' cellspacing='0'><TR>\n";		
		$html.="<TD class='columna-titulo-tabla'>$titulo</TD>\n";
		$html.="<TD align='right'><a href=\"javascript:obtenerXLS('$id_grupo','$id_tabla');\"><img src='".img_global."exp_xls.gif' border='0' alt='Exportar a EXCEL'></a></TD>\n";
		$html.="</TR></table></TD></TR>\n";
		$html.= $this->html_cabecera;
		$fila_actual = 0;
		foreach ($tabla['datos'] as $clave => $fila)
		{
			...
			{
				if(($this->navegar)&&($id=="0"))
				//Columna de descripcion
				{
					$estilo = $this->formato_columnas[$id]["estilo"];
					$formateador = "formato_" . $this->formato_columnas[$id]["formato"];
					$html.= "<td class='columna-titulo-1'>\n
							<TABLE width='100%' cellpadding='1' cellspacing='0'>\n
								<TR>\n
									<TD width='2%'><a href=\"javascript:drillDown('$clave');\"><img src='".img_global."navegar.gif' border='0' alt='Drill Down'></a></TD>\n							
									<TD width='98%' class='dato-descripcion-b'>" . $this->$formateador($columna) . "</TD>\n
								</TR>\n
							</TABLE>\n
							</td>\n";
				}else
				//Columna de datos
				{
					$estilo = $this->formato_columnas[$id]["estilo"];
					$formateador = "formato_" . $this->formato_columnas[$id]["formato"];
					$html.= "<td class='". $estilo ."'>" . $this->$formateador($columna) . "</td>\n";
				}
			}
			//Total X.
			if ($this->dir_total_x){
				$formateador = "formato_" . $this->dir_total_x_formato;
				$html .="<TD class='total-1'>".$this->$formateador($tabla['total_x'][$clave])."</TD>\n";
			}
			//Popup grafico FILAS
			if ($this->graf_filas) $html .="<td class='columna-titulo-2'><a href=\"javascript:popup_grafico('0','{$this->graf_datos_fc_indice}','$fila_actual');\"><img width='18' heigth='18' src='".img_global."chart.png' border='0' alt='Desplegar los datos de la\n fila en un gráfico'><a></td>\n";
			$html.= "</tr>\n";
			$fila_actual++;
		}
		//Total Y.
		if ($this->dir_total_y){
			$suma = 0;
			$html.= "<tr>\n";
			$html .="<TD class='columna-titulo-0'>Total</TD>\n";
			for($a=1;$a<=count($tabla['total_y']);$a++)
			{
				$suma += $tabla['total_y'][$a];
				$formateador = "formato_" . $this->formato_columnas[$a]["formato"];
				$html.= "<td class='total-1'>" . $this->$formateador($tabla['total_y'][$a]) . "</td>\n";
			}
			if ($this->dir_total_x){
				$formateador = "formato_" . $this->dir_total_x_formato;
				$html.= "<td class='total-0'>".$this->$formateador($suma)."</td>\n";
			}
			$html.= "</tr>\n";		
		}
		...
		if ($this->graf_columnas){
			...
			{
				$html.= "<td class='columna-titulo-2'><a href=\"javascript:popup_grafico('1','{$this->graf_datos_fc_indice}','$a');\"><img width='18' heigth='18' src='".img_global."chart.png' border='0' alt='Desplegar los datos de la\n columna en un gráfico'><a></td>\n";
			}
			if ($this->dir_total_x){
				$html.= "<td>&nbsp;</td>\n";
			}
...
//-------------------------------------> Manejo de GRAFICOS !!!! --------------
		...
		if(($this->graf_gen_tipo!="NO ACTIVADO")||($this->graf_columnas)||($this->graf_filas))
		{
			...
			$this->graf_datos_fc[0][$this->graf_datos_fc_indice]=$canal->generar_vinculo("siu_serv_grafico_popup",$grafico);
			...
			//Cargo los datos serializados para los graficos FC
			$this->graf_datos_fc[1][$this->graf_datos_fc_indice]=$canal->generar_vinculo("siu_serv_grafico_popup",$grafico_i);
...
//---------------------- SALIDA -----------------
			...
			if(($this->graf_gen_tipo!="NO ACTIVADO")||($this->graf_gen_tipo=="")){
				if(!$this->graf_gen_invertir){
					$html.= servicios::grafico($grafico,false);
				}else{
					$html.= servicios::grafico($grafico_i,false);
				}
				$html.="<br>\n";
	...
	function generar_html_nivel_1($grupo,$id_grupo="null")
	{
		...
		foreach($grupo as $id_tabla => $tabla)
		{
			...
			$html .= $this->generar_html_nivel_0($tabla,$id_grupo,$id_tabla,$titulo);
			$html.="<br>\n";
		}
		return $html;
	}
	...
	function generar_html_nivel_2($bloque)
	{
		...
		{
			$html  .= "<h1>".$descripcion . ": " . $this->arbol_titulos_CdC[$posicion_titulo][$id_grupo]."<h1>\n";
			$html  .= $this->generar_html_nivel_1($grupo,$id_grupo);
		}
		return $html;
	}
	//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	function generar_categoria_x($etiqueta, $columna, $estilo)
	{
		if($this->ordenable){
			return "<td class='columna-titulo-$estilo'>\n
					<TABLE width='100%' cellpadding='0' cellspacing='0'>\n
						<TR>\n
							...
							<TABLE width='100%' cellpadding='0' cellspacing='0'>\n
								<TR>\n
									<TD class='columna-titulo-$estilo'><a href=\"javascript:ordenar('asc','$columna');\"><img src='".img_global."orden_asc.gif' border='0' alt='Orden ascendente'></a></TD>\n
								</TR>\n															
								<TR>\n
									<TD class='columna-titulo-$estilo'><a href=\"javascript:ordenar('des','$columna');\"><img src='".img_global."orden_des.gif' border='0' alt='Orden descendente'></a></TD>\n
								</TR>\n
							</TABLE>															
							</TD>\n
						</TR>\n
			...
			return "<TD class='columna-titulo-$estilo'>$etiqueta</TD>\n";			
		}
	}
	//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
