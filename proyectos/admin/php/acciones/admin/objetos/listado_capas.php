...
<?
		...
		$parametros["sql"] = "SELECT DISTINCT c.clase, c.clase 
								FROM 	apex_clase c,
										apex_clase_tipo t
								WHERE 	t.clase_tipo = c.clase_tipo
								AND		(c.proyecto = '".$this->hilo->obtener_proyecto()."' OR c.proyecto='toba')
...
if (!in_array($rs->fields['cla_id'], dao_editores::get_clases_validas())) { ?>
          <td width="1%" class='lista-obj-botones<? echo $clase ?>'>
			<a href="<? echo $this->vinculador->generar_solicitud('admin',"/admin/objetos/propiedades",array( apex_hilo_qs_zona => $rs->fields['pro_id'] .apex_qs_separador. $rs->fields["obj_id"] ),false,false,null,true)?>" target="<? echo  apex_frame_centro ?>" class='basico'>
