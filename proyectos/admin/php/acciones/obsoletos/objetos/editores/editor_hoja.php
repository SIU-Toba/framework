<?
	if($editable = $this->zona->get_editable()){
		...
		if($abm_cabecera > -1){
			...
			$clave_ef = array("objeto_hoja"=>$editable[1]);
			...
			if($etapa!="SA")
			{
				...
				if($listado > -1){
					...
					if($abm_detalle > -1){
						$cargar_ef = array("objeto_hoja"=>$editable[1]);
						...
						//LISTA
						$where = array("(objeto_hoja_proyecto = '".$editable[0]."')",
										"(objeto_hoja = '".$editable[1]."')");
