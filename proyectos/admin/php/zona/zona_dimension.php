...
class zona_dimension extends zona
{
	...
	//Carga el EDITABLE que se va a manejar dentro de la ZONA
	{
		...
		if(!$rs){
			throw new excepcion_toba("ZONA-DIMENSION: NO se pudo cargar el editable ".$clave[0].",".$clave[1]." - [SQL]  $sql - [ERROR] " . $db["instancia"][apex_db_con]->ErrorMsg() );
			return false;
		}elseif($rs->EOF){
...
//		echo "&nbsp;".$this->editable_id[1]."@".$this->editable_id[0]."&nbsp;";
	...
	{	
			...
			($this->editable_info['dimension_tipo'])=="combo_db_restric")
		{
			...
			echo $this->solicitud->vinculador->generar_solicitud('admin',"/admin/dimensiones/tabla_restric",null,true,false,$param);
			echo "</a>";
			echo "</td>";
		}
	}
