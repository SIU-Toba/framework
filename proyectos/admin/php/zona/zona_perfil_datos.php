...
{
	function zona_usuario_perfil($id,$proyecto,&$solicitud)
	{
	...
	//Carga el EDITABLE que se va a manejar dentro de la ZONA
	{
		...
		if(!isset($editable)){
			...
			$clave[1] = $editable['usuario_perfil_datos'];
		}
		$sql = 	"	SELECT	*
					FROM	apex_usuario_perfil_datos
					WHERE	proyecto = '{$clave[0]}'
					AND		usuario_perfil_datos = '{$clave[1]}';";
		//echo $sql;
		$rs =& $db["instancia"][apex_db_con]->Execute($sql);
		if(!$rs){
			monitor::evento("bug","ZONA - GRUPO ACCESO: NO se pudo cargar el editable ".$clave[0].",".$clave[1]." - [SQL]  $sql - [ERROR] " . $db["instancia"][apex_db_con]->ErrorMsg() );
			return false;
		}elseif($rs->EOF){
