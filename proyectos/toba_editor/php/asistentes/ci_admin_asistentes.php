...
class ci_admin_asistentes extends toba_ci
{
    ...
    public function cargar_editor_molde($forzar = false)
    {
    ...
    public function conf__form_tipo_operacion(toba_ei_formulario $form)
    {
        if (toba::zona()->cargada()) {
    ...
    public function evt__form_tipo_operacion__modificacion($datos)
    {
    ...
    public function evt__siguiente_editar()
    {
        $this->set_pantalla('pant_edicion');
    }
    ...
    public function conf__pant_edicion()
    {
    ...
    public function evt__volver_editar()
    {
        $this->set_pantalla('pant_tipo_operacion');
    }

    public function evt__siguiente_generar()
    {
    ...
    public function generacion_requiere_confirmacion()
    {
        try {
    ...
    public function conf__pant_confirmacion()
    {
        try {
    ...
    public function evt__volver_generar()
    {
        $this->set_pantalla('pant_edicion');
    }

    public function conf__form_generaciones($componente)
    {
        $componente->set_datos($this->asistente()->get_opciones_generacion());
    }

    public function evt__form_generaciones__modificacion($datos)
    {
        $this->s__opciones_generacion = $datos;
    }

    public function evt__generar()
    {
    ...
    public function asistente($reset = false)
    {
        $datos = null;
        ...
        if ($reset || !isset($this->asistente)) {
            if ($this->s__molde_preexistente) {
                $this->asistente = toba_catalogo_asistentes::cargar_por_molde(
                    $this->s__clave_molde['proyecto'],
                    $this->s__clave_molde['molde'],
                    $datos
                );
                $this->asistente->preparar_molde();
            } else {
                $this->asistente = toba_catalogo_asistentes::cargar_por_tipo_operacion(
                    $this->s__datos_asistente['operacion_tipo'],
                    $datos
                );
            }
        }
