...
class ci_moldes_alternativo extends toba_ci
{
	protected $s__molde;
	...
	function evt__form_tipo_molde__modificacion($datos)
	{
		$this->s__tipo_molde_nuevo = $datos['tipo'];
	}
	...
	{
		$datos = toba_info_editores::get_lista_tipo_molde($this->s__tipo_molde_nuevo);
		$pantalla->set_etiqueta("Moldes <em>{$datos['descripcion_corta']}</em>");	
	}
	
	function conf__cuadro_planes($componente)
	{
		return toba_info_editores::get_lista_moldes_existentes(null, $this->s__tipo_molde_nuevo);
	}
