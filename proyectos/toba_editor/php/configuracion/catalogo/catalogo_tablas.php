...
class catalogo_tablas
{
    ...
    public function __construct($proyecto = null, $fuente = null)
    {
    ...
    public function get_tablas_nuevas()
    {
        return $this->_tablas_nuevas;
    }

    public function get_tablas_actualizables()
    {
        return $this->_tablas_actualizables;
    }

    public function get_tablas_no_activas()
    {
        return $this->_dt_bloqueados;
    }

    public function get_tablas_preseleccionadas()
    {
    ...
    ///-----------------------------------------------------------------------------------------------------------------
    public function buscar_dt_lockeados()
    {
        $this->_dt_bloqueados = $this->get_dt_bloqueados();
    }

    public function quitar_desactivados()
    {
    ...
    //-------------------------------------------------------------------------------------------------------------------
    public function buscar_schemas_fuente()
    {
        $datos = array();
    ...
    public function buscar_lista_tablas()
    {
        $this->_tablas = array();
    ...
    public function get_dt_bloqueados()
    {
    ...
    public function get_columnas_base($tabla)
    {
        $resultado = array();
        try {
    ...
    public function get_columnas_dt($tabla)
    {
        $resultado = array();
        try {
    ...
    //----------------------------------------------------------------------------------------------------------------------
    public function cambios_a_realizar($cols_dt, $cols_base)
    {
        $cambios = array();
        ...
        foreach ($cols_base as $col) {
            ...
            if ($col['tipo'] == 'C') {
                $temp['largo'] = $col['longitud'];
            }
            if (isset($col['not_null'])) {
                $temp['no_nulo_db'] = (int) $col['not_null'];
            }
            if (isset($col['pk'])) {
                $temp['pk'] = (int) $col['pk'];
            }
            if (isset($col['secuencia']) && trim($col['secuencia']) != '') {
                $temp['secuencia'] = $col['secuencia'];
            }
        ...
        foreach ($cols_dt as $clave => $valor) {
            if ($valor['externa'] != '1') {
                ...
                if ($valor['tipo'] == 'C') {
                    $aux_dt[$clave]['largo'] = $valor['largo'];
                }
                $aux_dt[$clave]['no_nulo_db'] = (int) $valor['no_nulo_db'];
                $aux_dt[$clave]['pk'] = (int) $valor['pk'];
                if (isset($valor['secuencia']) && trim($valor['secuencia']) != '') {
                    $aux_dt[$clave]['secuencia'] = $valor['secuencia'];
                }
            }
        }
        ...
        if (! empty($cols_nuevas)) {
            $cambios['A'] = $cols_nuevas;
        }

        $cols_faltantes = array_diff_key($aux_dt, $aux_base);		// ! empty ===> Base menos columnas
        if (! empty($cols_faltantes)) {
            $cambios['B'] = $cols_faltantes;
        }
    ...
    //----------------------------------------------------------------------------------------------------------------------
    public function desactivar_no_procesadas($procesadas = array())
    {
    ...
    //----------------------------------------------------------------------------------------------------------------------
    public function confirmar_acciones($tabla_nombre)
    {
    ...
    public function get_id_objeto($tabla)
    {
        try {
