...
class ci_proyecto extends toba_ci
{
    ...
    public function evt__modificacion()
    {
    ...
    public function conf__basica()
    {
        $datos = $this->dependencia('datos')->get();
        if (isset($datos['item_inicio_sesion'])) {
            $datos['carpeta_inicio_sesion'] = toba_info_editores::get_carpeta_de_item(
                $datos['item_inicio_sesion'],
                $datos['proyecto']
            );
        }
        return $datos;
    ...
    public function evt__login__modificacion($datos)
    {
        $this->dependencia('datos')->set($datos);
    }
    ...
    public function conf__login()
    {
        ...
        if (isset($datos['item_pre_sesion'])) {
            $datos['carpeta_pre_sesion'] = toba_info_editores::get_carpeta_de_item(
                $datos['item_pre_sesion'],
                $datos['proyecto']
            );
        }
        return $datos;
    ...
    public function evt__nucleo__modificacion($datos)
    {
        $this->dependencia('datos')->set($datos);
    }

    public function conf__nucleo(toba_ei_formulario $form)
    {
        $parametros_edicion = array();
        $datos = $this->dependencia('datos')->get();
        if (isset($datos['item_set_sesion'])) {
            $datos['carpeta_set_sesion'] = toba_info_editores::get_carpeta_de_item(
                $datos['item_set_sesion'],
                $datos['proyecto']
            );
        }
    ...
    public function evt__version__modificacion($datos)
    {
        $this->dependencia('datos')->set($datos);
    }

    public function conf__version()
    {
