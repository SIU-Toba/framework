...
class ci_sincronizador_tablas extends toba_ci
{
    ...
    public function generar_listado_tablas_nuevas()
    {
        $resultado = array();
    ...
    public function generar_listado_tablas_modificadas()
    {
        $resultado = array();
        ...
        foreach ($modificables as $klave => $mod) {
            $cambios = '';
            if (isset($mod['A'])) {
                $cambios .= ' Columnas nuevas: <strong>' . implode(', ', array_keys($mod['A'])) . '</strong><br/>';
            }
            if (isset($mod['B'])) {
                $cambios .= ' Columnas a quitar: <strong>' . implode(', ', array_keys($mod['B'])) . '</strong><br/>';
            }
            if (isset($mod['M'])) {
    ...
    public function conf__cc_inicio_colapsado($claves)
    {
        $colapsa = true;
