...
class zona_dimension extends toba_zona
{
	...
	//Carga el EDITABLE que se va a manejar dentro de la ZONA
	{
		...
		if(!$rs){
			throw new toba_error("ZONA-DIMENSION: NO se pudo cargar el editable ".$this->editable_id[0].",".$this->editable_id[1]." - [SQL]  $sql - [ERROR] " . $db["instancia"][apex_db_con]->ErrorMsg() );
			return false;
		}elseif($rs->EOF){
...
//		echo "&nbsp;".$this->editable_id[1]."@".$this->editable_id[0]."&nbsp;";
	...
	//Genera la barra especifica inferior del EDITABLE
	{
		...
		$sql = 	"	SELECT	o.proyecto as				objeto_proyecto,
			...
			$rs =& $db["instancia"][apex_db_con]->Execute($sql);
			if(!$rs){
				throw new toba_error("BARRA INFERIOR editor DIMENSIONES: NO se pudo cargar definicion: $this->contexto['elemento']. - [SQL]  $sql - [ERROR] " . $db["instancia"][apex_db_con]->ErrorMsg() );
			}
			if(!$rs->EOF){
		...
		$sql = 	"	SELECT	p.proyecto as 					proyecto,
			...
			$rs =& $db["instancia"][apex_db_con]->Execute($sql);
			if(!$rs){
				throw new toba_error("BARRA INFERIOR editor DIMENSIONES: NO se pudo cargar definicion. - [SQL]  $sql - [ERROR] " . $db["instancia"][apex_db_con]->ErrorMsg() );
			}
			if(!$rs->EOF){
