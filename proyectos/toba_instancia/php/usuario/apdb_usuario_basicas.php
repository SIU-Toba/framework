...
class apdb_usuario_basicas extends toba_ap_tabla_db
{
	protected function evt__pre_update($id)
	{
		...
		//Para encriptarla
		if ($this->_datos[$id]['autentificacion'] != 'plano') {
			$usuario = $this->_datos[$id]['usuario'];
			$sql = "SELECT clave, autentificacion FROM apex_usuario WHERE usuario = '$usuario'";
			$rs = toba::db($this->fuente)->consultar($sql);
			if ($rs[0]['clave'] != $this->_datos[$id]['clave']
					|| $rs[0]['autentificacion'] != $this->_datos[$id]['autentificacion']) {
				$this->encriptar_clave($id, $this->_datos[$id]['autentificacion']);
			}
		}
	}
	...
	{
		if ($metodo != 'md5') {
			$this->_datos[$id]['clave'] = encriptar_con_sal($this->_datos[$id]['clave'], $metodo);
		} else {
			$this->_datos[$id]['clave'] = hash($metodo, $id);			
		}
	}
}	
?>