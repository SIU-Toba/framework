...
class extension_cuadro_full extends toba_ei_cuadro
{
    ...
    */
    public function sumarizar_cc__departamento__prom_hab_loc($filas)
    {
        $habitantes = 0;
    ...
    */
    public function sumarizar_cc__zona__loc($filas)
    {
        return count($filas);
    }
    ...
    */
    public function sumarizar_cc__zona__habkil($filas)
    {
        $habitantes = 0;
        $superficie = 0;
        foreach ($filas as $fila) {
    ...
    public function html_cabecera_cc_contenido__zona(&$nodo)
    {
    ...
    public function html_pie_cc_contenido__zona(&$nodo)
    {
    ...
    public function html_pie_cc_cabecera__departamento(&$nodo)
    {
        $escapador = toba::escaper();
    ...
    public function pdf_cabecera_cc_contenido__zona(&$nodo)
    {
        $zona = $nodo['descripcion']['zona'];
        $locs = count($nodo['filas']);
        $deps = count($nodo['hijos']);
        $this->salida->texto(
            "<b>Zona</b>: $zona - <b>Departamentos</b>: $deps - <b>Localidades</b>: $locs",
            9,
            array('justification' => 'left')
        );
    }

    public function pdf_pie_cc_contenido__zona(&$nodo)
    {
        //Preparo una descripcion
        $zona = $nodo['descripcion']['zona'];
        $locs = count($nodo['filas']);
    ...
    public function pdf_pie_cc_cabecera__departamento(&$nodo)
    {
