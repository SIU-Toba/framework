...
class control_runtime extends toba_ci
{
    ...
    public function evt__modificar()
    {
        $this->modificar = true;
    ...
    public function conf_evt__cuadro__a($evento, $fila)
    {
        if ($this->modificar) {
            if ($fila === 1) {
    ...
    public function conf__multilinea()
    {
        return $this->datos;
    }

    public function conf_evt__multilinea__a($evento, $fila)
    {
        if ($this->modificar) {
            if ($fila === 1) {
                $evento->set_imagen('nucleo/agregar.gif');
