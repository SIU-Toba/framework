...
class Flickr extends Flickr_API
{
    ...
    public function getFrobToken($frob)
    {
    ...
    public function getPhotoURL($p, $size = 's', $ext = 'jpg')
    {
        return "http://static.flickr.com/{$p['server']}/{$p['id']}_{$p['secret']}_{$size}.{$ext}";
    }

    public function authCheckToken($token)
    {
    ...
    public function contactsGetList($filter = '')
    {
        $params = array('auth_token' => $this->token);
        if ($filter) {
    ...
    public function contactsGetPublicList($id)
    {
    ...
    public function blogsPostPhoto($blog_id, $photo_id, $title, $description, $blog_password = '')
    {
        $params = array('auth_token'  => $this->token,
                        'blog_id'     => $blog_id,
                        'photo_id'    => $photo_id,
    ...
    public function favoritesAdd($photo_id)
    {
        $params = array('auth_token'  => $this->token,
                        'photo_id'    => $photo_id);
    ...
    public function favoritesRemove($photo_id)
    {
        $params = array('auth_token'  => $this->token,
                        'photo_id'    => $photo_id);
    ...
    public function favoritesGetList($user_id = '', $extras = '', $per_page = '', $page = '')
    {
        $params = array('auth_token'  => $this->token);
        if ($user_id) {
    ...
    public function favoritesGetPublicList($user_id = '', $extras = '', $per_page = '', $page = '')
    {
        $params = array();
        if ($user_id) {
            $params['user_id']  = $user_id;
    ...
    public function peopleGetPublicPhotos($nsid, $extras = '', $per_page = '', $page = '')
    {
        $params = array('user_id' => $nsid);
        if ($extras) {
            $params['extras']   = $extras;
    ...
    public function peopleFindByEmail($email, &$username = '')
    {
    ...
    public function urlsLookupUser($url, &$username = '')
    {
    ...
    public function photosetsGetList($nsid = '')
    {
        $params = array();
        if ($this->token) {
        ...
        foreach ($xml->photosets->attributes() as $k => $v) {
            $ret[$k] = (string)$v;
        }
        $i = 0;
        foreach ($xml->photosets->photoset as $k => $v) {
            foreach ($v->attributes() as $kk => $vv) {
                $ret['photosets'][(string)$v['id']][$kk] = (string)$vv;
            }
    ...
    public function photosetsCreate($title, $primary_photo_id, $description = '')
    {
        $params = array('auth_token'  => $this->token);
        $params['title'] = $title;
    ...
    public function photosetsDelete($photoset_id)
    {
        $params = array('auth_token'  => $this->token);
    ...
    public function photosetsGetPhotos($photoset_id)
    {
        $params = array('photoset_id' => $photoset_id);
    ...
    public function photosetsAddPhoto($photoset_id, $photo_id)
    {
        $params = array('auth_token'  => $this->token);
        $params['photoset_id'] = $photoset_id;
    ...
    public function photosetsRemovePhoto($photoset_id, $photo_id)
    {
        $params = array('auth_token'  => $this->token);
        $params['photoset_id'] = $photoset_id;
        $params['photo_id'] = $photo_id;
    ...
    public function photosGetNotInSet($extras = '', $perpage = '', $page = '')
    {
        $params = array('auth_token'  => $this->token);
        if ($extras) {
            $params['extras']   = $extras;
    ...
    public function photosGetRecent($extras = '', $per_page = '', $page = '')
    {
        $params = array();
        if ($extras) {
            $params['extras']   = $extras;
    ...
    public function photosGetInfo($photo_id, $secret = '')
    {
        $params = array();
        if ($this->token) {
            $params['auth_token'] = $this->token;
    ...
    public function upload($photo, $title = '', $description = '', $tags = '', $perms = '', $async = 1, &$info = null)
    {
        $tmpf = false;
        $params = array('auth_token'  => $this->token);
        $url = parse_url($photo);
    ...
    public function photosUploadCheckTickets($tickets)
    {
        if (is_array($tickets)) {
    ...
    public function photosNotesAdd($photo_id, $x, $y, $w, $h, $text)
    {
        $params = array('auth_token'  => $this->token);
        $params['photo_id'] = $photo_id;
        $params['note_x'] = $x;
    ...
    public function photosNotesEdit($node_id, $x, $y, $w, $h, $text)
    {
        $params = array('auth_token'  => $this->token);
    ...
    public function photosNotesDelete($note_id)
    {
        $params = array('auth_token'  => $this->token);
    ...
    public function reflectionGetMethods()
    {
        $params = array();
