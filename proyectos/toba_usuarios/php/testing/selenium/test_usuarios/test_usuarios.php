...
{		
	...
	function test_permisos_y_perfiles()
	{
		...
		$botones = Titiritero::get_elementos($session, 'xpath', "//button[contains(@name, '_seleccion')]"); //*[@id="cuadro_2186_cuadro_proyectos1_seleccion"] 
		$botones[1]->click();
		$session->wait(2);
		
		Titiritero::setear_opcion_combo($session, 'xpath', '//select[contains(@name, "usuario_grupo_acc_izq")]', 1);
		Titiritero::click_element($session, 'xpath', '//select[contains(@name, "usuario_grupo_acc_izq")]//following::img[1]');		
		$session->wait(1);
	...
	function test_agrego_perfiles()
	{
		...
		$botones = Titiritero::get_elementos($session, 'xpath', "//button[contains(@name, '_seleccion')]"); 
		$botones[1]->click();
		$session->wait(2);
		
		Titiritero::setear_opcion_combo($session, 'xpath', '//select[contains(@name, "usuario_perfil_datos")]', 0);
		Titiritero::setear_opcion_combo($session, 'xpath', '//select[contains(@name, "usuario_grupo_acc_izq")]', 0);		
		Titiritero::click_element($session, 'xpath', '//select[contains(@name, "usuario_grupo_acc_izq")]//following::img[1]');		
		$session->wait(1);
		Titiritero::click_element($session, 'id', 'form_2187_form_proyectos_modificacion');
		$session->wait(2);
	...
	function test_quito_perfiles_y_permisos()
	{
		...
		Titiritero::click_element($session, 'id', 'form_2187_form_proyectos_baja');
