...
class RegistryHooksProyectoToba implements HooksInterface
{
     ...
     * configurar el sistema a partir de la informaci贸n en registry.
     ...
     * los datos dependientes de la instalaci贸n.
     * @param Provision $f una feature sin informaci贸n de instalaci贸n, por ej: url.
     */
    ...
    protected function postConsumeApi(Consumption $feature)
    {
        ...
        if ($feature->getName() == "api:siu/arai-usuarios") {
            ...
            echo "Activando v铆nculo entre 'toba_usuarios' y 'Ara铆' para la gesti贸n de cuentas\n";
        }
    }
    ...
    protected function postConsumeSamlIdp(Consumption $feature)
    {
        ...
        if (isset($options['certificate'])) {
            ...
            if (false === file_put_contents($certFile, $options['certificate'])) {
                throw new \Exception("No se pudo escribir el archivo $certFile. 驴Problemas de permisos?");
            }
    ...
    protected function preProvideApi(Provision $feature)
    {
        ...
        if (isset($_SERVER['ARAI_REGISTRY_ENDPOINT_BASE'])) {
            if (filter_var($_SERVER['ARAI_REGISTRY_ENDPOINT_BASE'], FILTER_VALIDATE_URL)) {
                ...
                echo "La URL especificada en env 'ARAI_REGISTRY_ENDPOINT_BASE' no es v谩lida\n";
            }
        }
       ...
       $posibles = explode(',' , $provider->getOptions()['auth']['type']);
        if (count($posibles) > 1) {
            if (! isset($datos['auth_tipo'])) {
                ...
                throw new \Exception('El servidor no provee el mecanismo de autenticaci髇 "' . $datos['auth_tipo'] . '" por favor seleccione otro' );
