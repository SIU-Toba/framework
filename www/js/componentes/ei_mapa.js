...
function msMap(DivTag, ControlType, p_protocol)
{
	...
	var _map_w   = parseInt(exp_style.exec( _tagMain.style.width )[1]);
	var _map_h   = parseInt(exp_style.exec( _tagMain.style.height )[1]);
	...
	var _iconLoading = imgDir + '/report_x.png';
  ...
  // internamente per impostare il reference di un'altra mappa.
	...
	this.setFullExtent = function(Xmin, Xmax, Ymin)
	{
		...
		if (_read_cookie == false) {// 20060316 (IE bugfix)
	...
	this.setZoomboxWH = function(x, y)
	{
		...
		_tagZoombox.style.width  = max(_zoombox_x_last,  _zoombox_x_first) - min(_zoombox_x_last,  _zoombox_x_first) + 'px';
		_tagZoombox.style.height = max(_zoombox_y_last,  _zoombox_y_first) - min(_zoombox_y_last,  _zoombox_y_first) + 'px';
...
// pointOverlay class prototype
	...
	function msIcon( p_img, p_shd, p_offsetX, p_offsetY )
	{
		...
		if (_img_name == '') {_img_name=imgDir + '/pixel.gif';}
		if (_shd_name == '') {_shd_name=imgDir + '/pixel.gif';}
